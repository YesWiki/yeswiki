    ...
    public function show()
    {
        ...
        switch ($this->step) {
            case 'default':
                ...
                echo $this->twig->render('installation.twig', $options);
                break;
            ...
            case 'install':
                ...
                if ($databaseMessages['VERIFY_MYSQL_PASSWORD']['result'] === 'danger') { break; }
                ...
                if ($databaseMessages['TEST_MYSQL_CONNECTION']['result'] === 'danger') { break; }
                ...
                if ($databaseMessages['SEARCH_FOR_DATABASE']['result'] == 'warning') {
                    ...
                    if ($databaseMessages['TRYING_TO_CREATE_DATABASE']['result'] === 'danger') { break; }
                    $databaseMessages['SEARCH_AGAIN_FOR_DATABASE'] = $this->check(_t('SEARCH_AGAIN_FOR_DATABASE'));
                    if ($databaseMessages['SEARCH_AGAIN_FOR_DATABASE']['result'] === 'danger') { break; }
                }
                ...
                if ($databaseMessages['CHECK_EXISTING_TABLE_PREFIX']['result'] === 'danger') { break; }
                ...
                if ($databaseMessages['CREATION_OF_TABLES']['result'] === 'danger') { break; }

                $databaseMessages['INSERTION_OF_PAGES'] = $this->check(_t('INSERTION_OF_PAGES'), $options);
                if ($databaseMessages['INSERTION_OF_PAGES']['result'] === 'danger') { break; }

                $filesMessages['WRITE_ROBOT_TXT'] = $this->check(_t('WRITE_ROBOT_TXT'));
                if ($filesMessages['WRITE_ROBOT_TXT']['result'] === 'danger') { break; }

                $filesMessages['WRITE_CONFIG'] = $this->check(_t('WRITE_CONFIG'));
                if ($filesMessages['WRITE_CONFIG']['result'] === 'danger') { break; }
...
exit;
        ...
        $this->check(
            ...
            $fp = @fopen(\YesWiki\Init::configFile, 'w'), '', 0
        );
