...
if ($this->UserIsAdmin()) {
	...
	if ($resultsparam) {
		...
		$GLOBALS["wiki"]->InsertTriple($this->GetPageTag(), 'http://outils-reseaux.org/_vocabulary/voteconfig', json_encode($_POST), '', '');
		echo json_encode(array('message'=>'pas de donnees'));	
	}
} else {
	echo json_encode(array('result'=>'only for admins'));
}
?>
