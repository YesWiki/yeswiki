    ...
    public function run() {    
        ...
        foreach ($vForms as &$vForm) {
            ...
            foreach ($vTemplate as &$vField) {
                if ($vField[BazarField::FIELD_TYPE] == 'map') {
                    $this->report ('MapField Found (param 1, param2) : ' . $vField[BazarField::FIELD_NAME] .  ', ' . $vField[BazarField::FIELD_LABEL]);
                    
                    if ($vField[BazarField::FIELD_NAME] == 'bf_latitude' && $vField[BazarField::FIELD_LABEL] == 'bf_longitude') {                    
                        $this->report ('This field is NOT OK and needs to be updated');
                        
                        $vField[BazarField::FIELD_NAME] = 'bf_geolocation';
                        $vField[BazarField::FIELD_LABEL] = _t('BAZ_FORM_EDIT_GEO_LABEL');
                        
                           $vFormsToProcess [] = $vForm['bn_id_nature'];
                           
                           $vNeedUpdate = true;
                    }
                    else if ($vField[BazarField::FIELD_NAME] !== 'bf_geolocation') {
                    ...
                    if (trim ($vField[MapField::FIELD_AUTOCOMPLETE_OTHERS]) != '') { // FIELD_AUTOCOMPLETE_OTHERS
                        $vAutocompleteFieldnames = 
                        is_string($vField[MapField::FIELD_AUTOCOMPLETE_OTHERS])
                        ? $vField[MapField::FIELD_AUTOCOMPLETE_OTHERS]
                        : (
                            is_array($vField[MapField::FIELD_AUTOCOMPLETE_OTHERS])
                            ? implode('|', $vField[MapField::FIELD_AUTOCOMPLETE_OTHERS])
                            : ''
                        );
                        ...
                        if (count ($vAutocompleteOther) == 6) {
                            $vAutocompleteOther [MapField::FIELD_AUTOCOMPLETE_OTHERS_COUNTRY] = $vAutocompleteOther [MapField::FIELD_AUTOCOMPLETE_OTHERS_STATE]??'';
                            $vAutocompleteOther [MapField::FIELD_AUTOCOMPLETE_OTHERS_STATE] = '';
                            
                            $vField[MapField::FIELD_AUTOCOMPLETE_OTHERS] = implode ('|', $vAutocompleteOther);                                
                        }
                    }
                }                
            }
        ...
        foreach ($vFormsToProcess as $vForm) {
            ...
            foreach ($vEntries as &$vEntry) {
                ...
                if (!empty ($vEntry['body']['bf_geolocation'])) {
                    ...
                    if (!empty ($vEntry['body']['bf_geolocation']['latitude'])) $vLatitude = $vEntry['body']['bf_geolocation']['latitude'];
                    if (!empty ($vEntry['body']['bf_geolocation']['longitude'])) $vLongitude = $vEntry['body']['bf_geolocation']['longitude'];
                    if (!empty ($vEntry['body']['bf_geolocation']['geometries'])) $vGeometries = $vEntry['body']['bf_geolocation']['geometries'];
