...
export default {
    ...
    methods:{
        ...
        getDatatableOptions(){
            ...
            let dom = this.getTemplateFromSlot('dom',{})
            if (dom && dom.length > 0){
                options.dom = dom
        ...
        removeRows(dataTable,newIds){
            let entryIdsToRemove = Object.keys(this.displayedRows).filter((id)=>!newIds.includes(id))
            ...
            dataTable.rows((idx,data,node)=>{
                return data?.id === undefined || entryIdsToRemove.includes(data?.id)
            }).remove()
        },
