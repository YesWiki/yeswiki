const geolocationHelper = (function(){
    ...
    const eventDispatcher = {
        ...
        runOnceOrWait: async function(eventId,eventPrefix,asynFunc){
        ...
        methods: {
            ...
            getCountry(code){
                var foundData = this.countries.find((countryData)=>{
                    return countryData.id.toLowerCase() ===  code.toLowerCase()
                })
                if (foundData){
            ...
            getCountryData(country){
                var normalizedCountry = this.normalizeNFDDiacritic(country)
            
                var foundData = this.countries.find((countryData)=>{
                    return this.normalizeNFDDiacritic(countryData.name) === normalizedCountry
                })
                if (foundData){
                    return foundData
                } else {
