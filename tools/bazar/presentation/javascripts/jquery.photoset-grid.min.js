...
;(function (t, i, s, e) {
  ...
  ;((o.prototype = {
    ...
    _setupRows(i, s) {
      ...
      for (const a in this.rows) this.rows[a] = parseInt(this.rows[a], 10)
      const h = t(i).find('img')
      let r = 0
      ;(t.each(this.rows, (t, i) => {
        const s = r
        const e = r + i
        ;(h.slice(s, e).wrapAll(`<div class="photoset-row cols-${i}"></div>`),
          (r = e))
      }),
        t(i)
          .find('.photoset-row:not(:last-child)')
          .css({ 'margin-bottom': s.gutter }))
    },
    _setupColumns(s, e) {
      const o = this
      const n = function () {
        function o() {
          ...
          i !== t(s).attr('data-width') &&
            (n.each(function () {
              ...
              const s = i.height()
              const o = Math.floor(0.025 * s)
              ;(t(this).height(s - o),
                t(this)
                  .find('img')
                  .each(function () {
                    const i = `${0.5 * (s - t(this).height())}px`
        ...
        const h = t(s).find('.photoset-cell')
        const r = t(s).find('.cols-1 .photoset-cell')
        const l = t(s).find('.cols-2 .photoset-cell')
        const c = t(s).find('.cols-3 .photoset-cell')
        const d = t(s).find('.cols-4 .photoset-cell')
        const f = t(s).find('.cols-5 .photoset-cell')
  ...
  t.fn.imagesLoaded = function (i) {
    ...
    var a = this
    var r = t.isFunction(t.Deferred) ? t.Deferred() : 0
    var l = t.isFunction(r.notify)
    var c = a.find('img').add(a.filter('img'))
    var d = []
    var f = []
    var u = []
    return (
      t.isPlainObject(i) &&
        t.each(i, (t, s) => {
          t === 'callback' ? (i = s) : r && r[t](s)
        }),
      c.length
        ? c.bind('load.imagesLoaded error.imagesLoaded', o).each((i, s) => {
            const o = s.src
            const a = t.data(s, 'imagesLoaded')
            return a && a.src === o
              ? (n(s, a.isBroken), e)
              : s.complete && s.naturalWidth !== e
                ? (n(s, s.naturalWidth === 0 || s.naturalHeight === 0), e)
