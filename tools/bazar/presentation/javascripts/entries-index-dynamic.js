  ...
  new Vue({
    ...
    methods: {
      ...
      saveFiltersIntoHash() {
        ...
        if (this.currentSort.field) {
          hashParams.set(
            'sort',
            `${this.currentSort.field}:${this.currentSort.asc}`,
          );
        }
      ...
      initFiltersFromHash(filters, hash) {
        ...
        hash.split('&').forEach((combinaison) => {
          ...
          if (hashKey == 'q') {
            ...
            const val = this.sortOptions.find(
              (s) => s.field == field && s.asc == (order == 'true'),
            );
    ...
    mounted() {
      ...
      $.getJSON(wiki.url('?api/entries/bazarlist'), this.params, (data) => {
        // process the filters
        const lang =
          document.URL.split('&').find((x) => x.includes('lang')) || '';
        ...
        if (this.sortOptions.length > 0) {
          ...
          if (this.params.champ) {
            const sort = this.sortOptions.find(
              (o) =>
                o.field === this.params.champ &&
                o.asc === (this.params.ordre === 'asc'),
            );
