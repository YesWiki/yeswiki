  ...
  function BazarTagsInputService() {
    this.init = function () {
      if (
        ...
        bazarlistTagsInputsData.length < 1
      )
        return null
  ...
  const BazarTagsInputRefresh = {
    ...
    refresh(element) {
      ...
      if (propertyName && formId && concernedInput) {
        $.get(wiki.url(`api/forms/${formId}`), (data) => {
          if (data.prepared) {
            const fields =
              typeof data.prepared == 'object'
              ...
              if (field.propertyname && field.propertyname == propertyName) {
                ...
                if (options) {
                  ...
                  for (const key in parent.bazarlistTagsInputsData) {
                    previousbazarlistTagsInputsData[key] =
                      parent.bazarlistTagsInputsData[key]
