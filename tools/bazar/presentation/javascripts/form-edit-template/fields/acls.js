...
export default {
  ...
  renderInput(field) {
    return {
      field:
        field.askIfActivateComments == 1
          ? `<i class="far fa-comment-dots"></i> ${field.fieldlabel || _t('BAZ_ACTIVATE_COMMENTS')}`
          : '',
      onRender() {
        ...
        $(currentField)
          ...
          .change((event) => {
            ...
            if (
              nameInput.val().trim().length == 0 ||
              nameInput.val().trim() == 'bf_acls'
            ) {
              nameInput.val('bf_commentaires')
            }

            const visibleSelect = $(base).find(
              'select[name=askIfActivateComments]',
            )
            const selectedValue = visibleSelect.val()

            const subElements = $(base).find(
              '.form-group.fieldLabel-wrap,.form-group.hint-wrap,.form-group.name-wrap,.form-group.value-wrap',
            )
              ...
              if (
                ...
                (currentValue.length == 0 ||
                  (currentValue.length == 1 &&
                    currentValue.includes('comments-closed')))
              ) {
                commentInput.val([' + '])
              }
            } else {
        ...
        renderHelper.defineLabelHintForGroup(
          field,
          'fieldlabel',
          _t('BAZ_FORM_EDIT_COMMENTS_FIELD_ACTIVATE_HINT'),
        )
        renderHelper.defineLabelHintForGroup(
          field,
          'hint',
