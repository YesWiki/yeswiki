...
;(function ($, doc, win) {
      ...
      return [
        obj.filter(function (i) {
          ...
          if (thumbImg) {
            ...
            thumbSrc =
              thumbImg.getAttribute(that.options.thumbAttr) ||
    ...
    return {
      ...
      generate() {
        ...
        for (i = 0; i < len; i++) {
          ...
          elements.push(
            `<li${type}><a href="${link.href}"><img loading="lazy" src="${thumbSrc}" alt="${title}" title="${title}" /></a></li>`,
          )
        }
  ...
  APControl = {
    autoPlayTimer: false,
    play() {
      APControl.autoPlayTimer = setTimeout(() => {
        changeImage(nextImage)
      }, options.time)
  ...
  function getPrefixed(prop) {
    let i
    const s = doc.createElement('p').style
  ...
  function keyDown(event) {
    ...
    const ok = options.keys
    let result
    // Prevent default keyboard action (like navigating inside the page)
    return (
      ($.inArray(code, ok.close) >= 0 && close()) ||
      ($.inArray(code, ok.next) >= 0 &&
        !options.single &&
        loophole(nextImage)) ||
      ($.inArray(code, ok.prev) >= 0 &&
        !options.single &&
        loophole(prevImage)) ||
      true
    )
    ...
    save() {
      ...
      if (
        ...
        window.history.pushState(
          ...
          `${window.location.pathname + window.location.search}#${encodeURIComponent(activeURL)}`,
        )
      }
    },
    load() {
  ...
  function thumbsResize(e, delta) {
    ...
    const thumbList = photobox.thumbsList
    let h
  ...
  function close() {
    ...
    function hide() {
      ...
      options.hideFlash &&
        $('iframe, object, embed').css('visibility', 'visible')
    }
  ...
  !(function (a) {
    ...
    function b(b) {
      ...
      if (
        ...
        'deltaX' in g && ((l = g.deltaX), m === 0 && (j = -1 * l)),
        m !== 0 || l !== 0)
      ) {
        if (g.deltaMode === 1) {
          const q = a.data(this, 'mousewheel-line-height')
          ;((j *= q), (m *= q), (l *= q))
        } else if (g.deltaMode === 2) {
    ...
    function d(a, b) {
      return (
        k.settings.adjustOldDeltas && a.type === 'mousewheel' && b % 120 === 0
      )
    }
    let e
    let f
    ...
    if (a.event.fixHooks)
      for (let j = g.length; j; ) a.event.fixHooks[g[--j]] = a.event.mouseHooks
    var k = (a.event.special.mousewheel = {
      version: '3.1.11',
      setup() {
        if (this.addEventListener)
          for (let c = h.length; c; ) this.addEventListener(h[--c], b, !1)
      ...
      teardown() {
        if (this.removeEventListener)
          for (let c = h.length; c; ) this.removeEventListener(h[--c], b, !1)
        else this.onmousewheel = null
        ;(a.removeData(this, 'mousewheel-line-height'),
          a.removeData(this, 'mousewheel-page-height'))
      },
      getLineHeight(b) {
        let c = a(b)['offsetParent' in a.fn ? 'offsetParent' : 'parent']()
