...
class CalcField extends BazarField
{
    ...
    public function formatValuesBeforeSave($entry)
    {
        ...
        if (!preg_match_all("/($operators|$parenthesis)|($number)|($fieldPropertyName)|($functions)/", $this->calcFormula, $matches)) {
            $value = 0;
        } else {
            $formula = "";
            foreach ($matches[0] as $key => $value) {
                if (!empty($matches[1][$key]) && empty($matches[2][$key]) && empty($matches[3][$key]) && empty($matches[4][$key])) {
                    // operators or parenthesis
                    $formula .= $matches[1][$key];
                } elseif (empty($matches[1][$key]) && !empty($matches[2][$key]) && empty($matches[3][$key]) && empty($matches[4][$key])) {
                    // number
                    $formula .= floatval($matches[2][$key]);
                } elseif (empty($matches[1][$key]) && empty($matches[2][$key]) && !empty($matches[3][$key]) && empty($matches[4][$key])) {
                    // field property name
                    $formula .= $this->getEntryValue($entry, $matches[3][$key]);
                } elseif (empty($matches[1][$key]) && empty($matches[2][$key]) && empty($matches[3][$key]) && !empty($matches[4][$key])) {
                    // functions
                    $formula .= $matches[4][$key];
                }
            }
