      ...
      $.ajax({
        ...
        for (var idform in data) {
          if (data.hasOwnProperty(idform)) {
            count++;
            var trclass = '', existingmessage = "";
            if (existingforms.find('td').filter(function() {return $(this).find('strong').text() === data[idform].bn_label_nature;}).length>0) {
            	trclass = ' class="error danger"';
            	existingmessage = '<br><span class="text-danger">'+formtranslations.existingmessagereplace+'</span>';
          	} else if (existingforms.find('td').filter(function() {return $(this).text() === idform;}).length>0) {
