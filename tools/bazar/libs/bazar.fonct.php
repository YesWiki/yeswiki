...
// +------------------------------------------------------------------------------------------------------+
require_once
BAZ_CHEMIN . 'libs' . DIRECTORY_SEPARATOR . 'vendor' . DIRECTORY_SEPARATOR .
'HTML/QuickForm.php';
require_once
BAZ_CHEMIN . 'libs' . DIRECTORY_SEPARATOR . 'vendor' . DIRECTORY_SEPARATOR .
...
require_once
BAZ_CHEMIN . 'libs' . DIRECTORY_SEPARATOR . 'formulaire' . DIRECTORY_SEPARATOR
 ...
 */
function baz_afficher_formulaire_fiche($mode, $formtemplate, $url = '',
    $valeurs = '') {
    $res = '';
    ...
    if ($form['bn_condition'] != '' && !isset($_POST['accept_condition']) &&
        ...
        require_once
        BAZ_CHEMIN . 'libs' . DIRECTORY_SEPARATOR . 'vendor' .
...
function baz_requete_bazar_fiche($valeur)
{
    ...
    if (count($matches[0]) > 0) {
        $valeur = array_merge($valeur, titre($formtemplate, array('titre',
            $valeur['bf_titre']), 'requete', $valeur));
    }
    ...
    if (!isset($valeur['id_fiche'])) {
        ...
        $valeur['id_fiche'] = genere_nom_wiki($valeur['bf_titre']);
        $_POST['id_fiche']  = $valeur['id_fiche'];
    }

    //createur de la fiche
    if ($GLOBALS['wiki']->GetPageOwner($valeur['id_fiche'])) {
        $valeur['createur'] = $GLOBALS['wiki']
            ->GetPageOwner($valeur['id_fiche']);
    ...
    $datecreation = $GLOBALS['wiki']->LoadSingle(
        ...
        "pages WHERE tag='" . $valeur['id_fiche'] . "'"
    );
    $valeur['date_creation_fiche'] = $datecreation['firsttime'] ?
    ...
    if (isset($destmail)) {
        ...
        $fiche = str_replace('src="tools', 'src="' . $lien . '/tools',
            baz_voir_fiche(0, $valeur)) . $texthtml;
        $html =
        '<html><head><style type="text/css">' . $style .
        '</style></head><body>' . $fiche . '</body></html>';

        send_mail(BAZ_ADRESSE_MAIL_ADMIN, BAZ_ADRESSE_MAIL_ADMIN, $destmail,
            $sujet, $text, $html);
    }
...
function baz_mise_a_jour_fiche($valeur)
{
    ...
    if (BAZ_ENVOI_MAIL_ADMIN) {
        ...
        $style   = str_replace('url(',
            'url(' . $lien . '/tools/bazar/presentation/', $style);
        $fiche = str_replace('src="tools', 'src="' . $lien . '/tools',
            baz_voir_fiche(0, $valeur['id_fiche'])) . $texthtml;
        $html =
        '<html><head><style type="text/css">' . $style .
        '</style></head><body>' . $fiche . '</body></html>';
...
function baz_suppression($idfiche)
{
    if ($idfiche != '') {
        ...
        if (baz_a_le_droit('saisie_fiche', $valeur['bf_ce_utilisateur'])) {
            ...
            $GLOBALS['wiki']->DeleteTriple($idfiche,
                'http://outils-reseaux.org/_vocabulary/type', null, '', '');
...
function baz_liste_rss()
{
    ...
    foreach ($resultat as $categorie) {
        foreach ($categorie as $ligne) {
            $liste .= '<li><a href="' . $GLOBALS['wiki']->href('rss', '',
                'id_typeannonce=' . $ligne['bn_id_nature'])
...
function baz_formulaire_des_formulaires($mode, $valeursformulaire = '')
{
    ...
    if (isset($_GET['idformulaire'])) {
        $formtemplate->addElement('hidden', 'bn_id_nature',
            $_GET['idformulaire']);
    }
    ...
    //champs obligatoires
    $formtemplate->addRule('bn_label_nature', _t('BAZ_CHAMPS_REQUIS')
        . ' : ' . _t('BAZ_FORMULAIRE'), 'required', '', 'client');
    $formtemplate->addRule('bn_template',
        _t('BAZ_CHAMPS_REQUIS') . ' : ' . _t('BAZ_TEMPLATE'), 'required', '',
        'client');
...
function multiArraySearch($array, $key, $value)
{
    ...
    if (is_array($array)) {
        ...
        foreach ($array as $subarray) {
            $results = array_merge($results, multiArraySearch($subarray, $key,
                $value));
        }
    }
...
function baz_gestion_formulaire()
{
    ...
    if ($GLOBALS['wiki']->GetUser()) {
        ...
        if (isset($_GET['action_formulaire']) && $_GET['action_formulaire'] ==
            ...
            'nature (`bn_id_nature` ,`bn_ce_i18n` ,`bn_label_nature` ,`bn_template` ,`bn_description` ,`bn_condition`, `bn_label_class` ,`bn_type_fiche`)' . ' VALUES (' . baz_nextId(BAZ_PREFIXE . 'nature', 'bn_id_nature',
                $GLOBALS['wiki']) . ', "fr-FR", "' .
            addslashes(_convert($_POST['bn_label_nature'],
                TEMPLATES_DEFAULT_CHARSET,
                true)) . '", "' . addslashes(_convert($_POST['bn_template'],
                TEMPLATES_DEFAULT_CHARSET,
                true)) . '", "' .
            addslashes(_convert($_POST['bn_description'],
                TEMPLATES_DEFAULT_CHARSET,
                true)) . '", "' . addslashes(_convert($_POST['bn_condition'],
                TEMPLATES_DEFAULT_CHARSET,
                true)) . '", "' .
            addslashes(_convert($_POST['bn_label_class'],
                TEMPLATES_DEFAULT_CHARSET,
                true)) . '", "' . addslashes(_convert($_POST['bn_type_fiche'],
                TEMPLATES_DEFAULT_CHARSET,
                true)) . '")';
            $resultat = $GLOBALS['wiki']->query($requete);
            ...
            $requete =
            'UPDATE ' . BAZ_PREFIXE . 'nature SET `bn_label_nature`="' .
            addslashes(_convert($_POST['bn_label_nature'],
                TEMPLATES_DEFAULT_CHARSET,
                true)) . '" ,`bn_template`="' .
            addslashes(_convert($_POST['bn_template'],
                TEMPLATES_DEFAULT_CHARSET,
                true)) . '" ,`bn_description`="' .
            addslashes(_convert($_POST['bn_description'],
                TEMPLATES_DEFAULT_CHARSET,
                true)) . '" ,`bn_condition`="' .
            addslashes(_convert($_POST['bn_condition'],
                TEMPLATES_DEFAULT_CHARSET,
                true)) . '" ,`bn_label_class`="' .
            addslashes(_convert($_POST['bn_label_class'],
                TEMPLATES_DEFAULT_CHARSET,
                true)) . '" ,`bn_type_fiche`="' .
            addslashes(_convert($_POST['bn_type_fiche'],
                TEMPLATES_DEFAULT_CHARSET,
                true)) . '"' . ' WHERE `bn_id_nature`=' .
            $_POST['bn_id_nature'];
            $resultat = $GLOBALS['wiki']->query($requete);
        ...
        if (!isset($_GET['action_formulaire']) ||
            ...
            if (isset($_POST['imported-form'])) {
                foreach ($_POST['imported-form'] as $id => $value) {
                    ...
                    $searchformname = multiArraySearch($forms,
                        'bn_label_nature', $value['bn_label_nature']);
                    ...
                    if (count($searchformname) > 0) {
                        ...
                        $requete               =
                        'UPDATE ' . BAZ_PREFIXE .
                        'nature SET `bn_label_nature`="' .
                        addslashes(_convert($value['bn_label_nature'],
                            TEMPLATES_DEFAULT_CHARSET,
                            true)) . '" ,`bn_template`="' .
                        addslashes(_convert($value['bn_template'],
                            TEMPLATES_DEFAULT_CHARSET,
                            true)) . '" ,`bn_description`="' .
                        addslashes(_convert($value['bn_description'],
                            TEMPLATES_DEFAULT_CHARSET,
                            true)) . '" ,`bn_condition`="' .
                        addslashes(_convert($value['bn_condition'],
                            TEMPLATES_DEFAULT_CHARSET,
                            true)) . '" ,`bn_label_class`="' .
                        addslashes(_convert($value['bn_label_class'],
                            TEMPLATES_DEFAULT_CHARSET,
                            true)) . '" ,`bn_type_fiche`="' .
                        addslashes(_convert($value['bn_type_fiche'],
                            TEMPLATES_DEFAULT_CHARSET,
                            true)) . '"' . ' WHERE `bn_id_nature`=' .
                        $value['bn_id_nature'];
                        ...
                        $searchformid = multiArraySearch($forms,
                            'bn_id_nature', $id);
                        if (count($searchformid) > 0) {
                            $id = baz_nextId(BAZ_PREFIXE . 'nature',
                                'bn_id_nature', $GLOBALS['wiki']);
                        }
                        $requete =
                        'INSERT INTO ' . BAZ_PREFIXE .
...
function baz_gestion_listes()
{
    ...
    if (!isset($_GET['action'])) {
        ...
        $res .= baz_formulaire_des_listes(BAZ_ACTION_MODIFIER_LISTE_V,
            $valeursliste);
        ...
        if (TEMPLATES_DEFAULT_CHARSET != 'UTF-8') {
            $valeur['label'] = array_map('utf8_encode',
                $valeur['label']);
        ...
        //on cree un triple pour specifier que la PageWiki creee est une liste
        $GLOBALS['wiki']->InsertTriple($nomwikiliste,
            'http://outils-reseaux.org/_vocabulary/type', 'liste', '', '');
        ...
        $GLOBALS['wiki']->Redirect(
            $GLOBALS['wiki']->href('', $GLOBALS['wiki']->GetPageTag(),
                BAZ_VARIABLE_VOIR . '=' . BAZ_VOIR_LISTES, false)
        );
        ...
        $GLOBALS['wiki']->Redirect($GLOBALS['wiki']->href('',
            $GLOBALS['wiki']->GetPageTag(), BAZ_VARIABLE_VOIR . '=' .
            BAZ_VOIR_LISTES, false));
...
function baz_valeurs_liste($idliste = '')
{
    if ($idliste != '') {
        if (!isset($GLOBALS['bazar']['lists'][$idliste])) {
            //on verifie que la page en question est bien une page wiki
            if ($GLOBALS['wiki']->GetTripleValue($idliste,
                'http://outils-reseaux.org/_vocabulary/type', '', '') ==
                'liste') {
...
function baz_voir_fiche($danslappli, $idfiche)
{
    ...
    if
    (file_exists('themes/tools/bazar/templates/fiche-' .
        $fichebazar['values']['id_typeannonce'] . '.tpl.html')) {
        // un template fiche existe
        include_once 'tools/bazar/libs/squelettephp.class.php';
        $templatetoload =
        'themes/tools/bazar/templates/fiche-' .
        $fichebazar['values']['id_typeannonce'] . '.tpl.html';
    ...
    if ($danslappli == 1) {
        ...
        $fichebazar['infos'] .=
        ', <span class="date_creation">' . _t('BAZ_DATE_CREATION') . ' '
        . strftime('%d.%m.%Y &agrave; %H:%M',
            strtotime($fichebazar['values']['date_creation_fiche'])) .
        '</span>';
        $fichebazar['infos'] .=
        ', <span class="date_mise_a_jour">' . _t('BAZ_DATE_MAJ') . ' '
        . strftime('%d.%m.%Y &agrave; %H:%M',
            strtotime($fichebazar['values']['date_maj_fiche'])) . '</span>.';
        ...
        if (baz_a_le_droit('saisie_fiche', $GLOBALS['wiki']
            ...
            $fichebazar['infos'] .=
            '<li><a class="btn btn-xs btn-mini btn-default" href="'
            ...
            $fichebazar['infos'] .=
            '<li><a class="btn btn-xs btn-mini btn-danger" href="'
            . $GLOBALS['wiki']->href('deletepage',
                $idfiche) . '" onclick="javascript:return confirm(\''
            . _t('BAZ_CONFIRM_SUPPRIMER_FICHE') . '\');">'

            .
            '<i class="glyphicon glyphicon-trash icon-trash icon-white"></i> '
            . _t('BAZ_SUPPRIMER') . '</a></li>' . "\n";
...
function baz_rechercher($typeannonce = '', $categorienature = '')
{
    ...
    $urlparam =
    BAZ_VARIABLE_VOIR . '=' . BAZ_VOIR_DEFAUT . '&' . BAZ_VARIABLE_ACTION .
    ...
    $data['search'] = isset($_REQUEST['recherche_mots_cles']) &&
    !empty($_REQUEST['recherche_mots_cles']) ?
    ...
    if (!isset($_REQUEST['id_typeannonce'])) {
        ...
        $tableau_dernieres_fiches = baz_requete_recherche_fiches('', '',
            $typeannonce, $categorienature, 1, '', 10);
        $res .= baz_afficher_liste_resultat($tableau_dernieres_fiches, false);
    } else {
 ...
 */
function baz_requete_recherche_fiches($tableau_criteres = '', $tri = '',
    $id_typeannonce = '', $categorie_fiche = '', $statut = 1, $personne = '',
    $nb_limite = '', $motcles = true, $searchstring = '', $facettesearch =
    'OR') {
    $nb_jointures = 0;

    //si les parametres ne sont pas rentres, on prend les variables globales
    /*if ($id_typeannonce == '' && isset($GLOBALS['params']['idtypeannonce'])) {
    if isset()
    $id_typeannonce = $GLOBALS['params']['idtypeannonce'];
    } elseif (!empty($id_typeannonce) && ) {}*/
    if ($categorie_fiche == '' &&
    ...
    if (isset($searchstring) && $searchstring != '' && $searchstring !=
        _t('BAZ_MOT_CLE')) {
        $searchstring = _convert($searchstring, TEMPLATES_DEFAULT_CHARSET,
            true);
...
function baz_afficher_liste_resultat($tableau_fiches, $info_nb = true)
{
    ...
    if ($info_nb) {
        ...
        if ($nb_result <= 1) {
            $fiches['info_res'] .=
            $nb_result . ' ' . _t('BAZ_FICHE_CORRESPONDANTE') . '</div>' .
            "\n";
        } else {
            $fiches['info_res'] .=
            $nb_result . ' ' . _t('BAZ_FICHES_CORRESPONDANTES') . '</div>' .
            "\n";
        }
    }
    ...
    BAZ_CHEMIN . 'libs' . DIRECTORY_SEPARATOR . 'vendor' . DIRECTORY_SEPARATOR
    . 'Pager/Pager.php';
    $param = array('mode' => BAZ_MODE_DIVISION, 'perPage'          =>
        BAZ_NOMBRE_RES_PAR_PAGE,
        'delta'               => BAZ_DELTA,
        'httpMethod'          => 'GET',
        'extraVars'           => array_merge($_POST, $_GET), 'altNext' =>
        _t('BAZ_SUIVANT'),
        'altPrev'             => _t('BAZ_PRECEDENT'), 'nextImg'        =>
        _t('BAZ_SUIVANT'), 'prevImg' =>
        _t('BAZ_PRECEDENT'),
        'itemData'            => $tableau_fiches);
    $pager                 = &Pager::factory($param);
    $data                  = $pager->getPageData();
    $links                 = $pager->getLinks();
    $fiches['pager_links'] = '<div class="bazar_numero">' . $pager
        ->links . '</div>' . "\n";
    ...
    foreach ($data as $fiche) {
        ...
        if (baz_a_le_droit('supp_fiche', (isset($valeurs_fiche['createur']) ?
            $valeurs_fiche['createur'] : ''))) {
            $valeurs_fiche['lien_suppression'] = '<a href="' . $GLOBALS['wiki']
                ->href('deletepage',
                    $valeurs_fiche['id_fiche']) .

            '" class="BAZ_lien_supprimer" onclick="javascript:return confirm(\'' .
            _t('BAZ_CONFIRM_SUPPRIMER_FICHE') . ' ?\');"></a>' . "\n";
        }
        if (baz_a_le_droit('modif_fiche', (isset($valeurs_fiche['createur']) ?
            $valeurs_fiche['createur'] : ''))) {
            $valeurs_fiche['lien_edition'] = '<a href="' . $GLOBALS['wiki']
                ->href('edit', $valeurs_fiche['id_fiche']) .
            '" class="BAZ_lien_modifier"></a>' . "\n";
        }
        $valeurs_fiche['lien_voir_titre'] = '<a href="' . $GLOBALS['wiki']
            ->href('',
                $valeurs_fiche['id_fiche']) .
        '" class="BAZ_lien_voir" title="Voir la fiche">' .
        stripslashes($valeurs_fiche['bf_titre']) . '</a>' . "\n" . '</li>' .
        "\n";
        $valeurs_fiche['lien_voir'] = '<a href="' . $GLOBALS['wiki']->href('',
            $valeurs_fiche['id_fiche']) .
        '" class="BAZ_lien_voir" title="Voir la fiche"></a>' . "\n" . '</li>'
        . "\n";
    ...
    $template = (isset($_GET['template']) &&
        (is_file('templates/bazar/' . $_GET['template'])
            || is_file('tools/bazar/presentation/templates/' .
                $_GET['template'])
            || is_file('themes/tools/bazar/templates/' . $_GET['template']))) ?
    $_GET['template'] : $GLOBALS['params']['template'];
...
function encoder_en_utf8($txt)
{
    ...
    return strtr(preg_replace('/ \x{0026} /u', ' &#38; ',
        mb_convert_encoding($txt, 'UTF-8', 'HTML-ENTITIES')), $cp1252_map);
}
...
function baz_afficher_flux_RSS()
{
    ...
    $xml .= XML_Util::createStartElement('rss', array('version' => '2.0',
        'xmlns:atom'                                                =>
        'http://www.w3.org/2005/Atom', 'xmlns:dc' =>
        'http://purl.org/dc/elements/1.1/'));
    $xml .= "\r\n    ";
    $xml .= XML_Util::createStartElement('channel');
    $xml .= "\r\n      ";
    $xml .= XML_Util::createTag('title', null,
        html_entity_decode(_t('BAZ_DERNIERE_ACTU'), ENT_QUOTES, 'UTF-8'));
    $xml .= "\r\n      ";
    $xml .= XML_Util::createTag('link', null,
        html_entity_decode(BAZ_RSS_ADRESSESITE, ENT_QUOTES, 'UTF-8'));
    $xml .= "\r\n      ";
    $xml .= XML_Util::createTag('description', null,
        html_entity_decode(BAZ_RSS_DESCRIPTIONSITE, ENT_QUOTES, 'UTF-8'));
    $xml .= "\r\n      ";
    $xml .= XML_Util::createTag('language', null, 'fr-FR');
    $xml .= "\r\n      ";
    $xml .= XML_Util::createTag('copyright', null,
        'Copyright (c) ' . date('Y') . ' ' .
        html_entity_decode(BAZ_RSS_NOMSITE, ENT_QUOTES, 'UTF-8'));
    $xml .= "\r\n      ";
    $xml .= XML_Util::createTag('lastBuildDate', null,
        strftime('%a, %d %b %Y %H:%M:%S GMT'));
    $xml .= "\r\n      ";
    $xml .= XML_Util::createTag('docs', null,
        'http://www.stervinou.com/projets/rss/');
    $xml .= "\r\n      ";
    $xml .= XML_Util::createTag('category', null, BAZ_RSS_CATEGORIE);
    $xml .= "\r\n      ";
    $xml .= XML_Util::createTag('managingEditor', null,
    ...
    if (count($tableau_flux_rss) > 0) {
        ...
        foreach ($tableau_flux_rss as $ligne) {
            ...
            $xml .= "\r\n        ";
            $xml .= XML_Util::createTag('title', null,
                html_entity_decode(stripslashes($ligne['bf_titre']),
                    ENT_QUOTES, 'UTF-8'));
            $xml .= "\r\n        ";
            ...
            $xml .= XML_Util::createTag('link', null, '<![CDATA[' . $lien
                    ->getURL() . ']]>');
            $xml .= "\r\n        ";
            $xml .= XML_Util::createTag('guid', null, '<![CDATA[' . $lien
                    ->getURL() . ']]>');
            $xml .= "\r\n        ";

            $xml .= XML_Util::createTag('dc:creator', null,
                $ligne['createur']);
            $xml .= "\r\n      ";

            $tab = explode('wakka.php?wiki=', $lien->getURL());
            $xml .= XML_Util::createTag('description', null,
                '<![CDATA[' . preg_replace('/data-id=".*"/Ui', '',
                    html_entity_decode(baz_voir_fiche(0,
                        $ligne), ENT_QUOTES, 'UTF-8')) . ']]>');
            $xml .= "\r\n        ";
            $xml .= XML_Util::createTag('pubDate', null,
                strftime('%a, %d %b %Y %H:%M:%S GMT',
                    strtotime($ligne['date_creation_fiche'])));
            $xml .= "\r\n      ";
        ...
        $xml .= "\r\n          ";
        $xml .= XML_Util::createTag('title', null,
            html_entity_decode(_t('BAZ_PAS_DE_FICHES'), ENT_QUOTES, 'UTF-8'));
        $xml .= "\r\n          ";
        $xml .= XML_Util::createTag('link', null,
            '<![CDATA[' . $GLOBALS['_BAZAR_']['url']->getUrl() . ']]>');
        $xml .= "\r\n          ";
        $xml .= XML_Util::createTag('guid', null,
            '<![CDATA[' . $GLOBALS['_BAZAR_']['url']->getUrl() . ']]>');
        $xml .= "\r\n          ";
        $xml .= XML_Util::createTag('description', null,
            html_entity_decode(_t('BAZ_PAS_DE_FICHES'), ENT_QUOTES, 'UTF-8'));
        $xml .= "\r\n          ";
        $xml .= XML_Util::createTag('pubDate', null,
            strftime('%a, %d %b %Y %H:%M:%S GMT', strtotime('01/01/%Y')));
        $xml .= "\r\n      ";
        $xml .= XML_Util::createEndElement('item');
    }
    $xml .= "\r\n    ";
    ...
    echo str_replace('</image>',
        ...
        '" rel="self" type="application/rss+xml" />', html_entity_decode($xml,
            ENT_QUOTES, 'UTF-8'));
}
...
function getAllParameters($wiki)
{
    ...
    if (isset($_REQUEST['id_typeannonce']) &&
        !empty($_REQUEST['id_typeannonce'])) {
        $param['idtypeannonce'] = $_REQUEST['id_typeannonce'];
    } else {
    ...
    $param['groups'] = $wiki->GetParameter('groups');
    ...
    $param['titles'] = $wiki->GetParameter('titles');
