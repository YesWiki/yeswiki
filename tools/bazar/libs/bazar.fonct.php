...
function baz_afficher_menu()
{
    ...
    if (strstr(BAZ_VOIR_AFFICHER, strval(BAZ_VOIR_MES_FICHES))) {
        ...
        if (isset($_GET[BAZ_VARIABLE_VOIR]) && $_GET[BAZ_VARIABLE_VOIR] == BAZ_VOIR_MES_FICHES) {
            $class = ' btn-primary';
        } else {
            $class = ' btn-default';
        }
        $res .= '><a href="'.str_replace('&', '&amp;', $GLOBALS['_BAZAR_']['url']->getURL()).'" class="btn'.$class.'">'.
    ...
    if (strstr(BAZ_VOIR_AFFICHER, strval(BAZ_VOIR_CONSULTER))) {
        ...
        if ((isset($_GET[BAZ_VARIABLE_VOIR]) && $_GET[BAZ_VARIABLE_VOIR] == BAZ_VOIR_CONSULTER)) {
            $class = ' btn-primary';
        } else {
            $class = ' btn-default';
        }
    ...
    if (strstr(BAZ_VOIR_AFFICHER, strval(BAZ_VOIR_SAISIR))) {
        ...
        if (isset($_GET[BAZ_VARIABLE_VOIR]) && ($_GET[BAZ_VARIABLE_VOIR] == BAZ_VOIR_SAISIR)) {
            $class = ' btn-primary';
        } else {
            $class = ' btn-default';
        }
    ...
    if (strstr(BAZ_VOIR_AFFICHER, strval(BAZ_VOIR_S_ABONNER))) {
        ...
        if (isset($_GET[BAZ_VARIABLE_VOIR]) && $_GET[BAZ_VARIABLE_VOIR] == BAZ_VOIR_S_ABONNER) {
            $class = ' btn-primary';
        } else {
            $class = ' btn-default';
        }
    ...
    if (strstr(BAZ_VOIR_AFFICHER, strval(BAZ_VOIR_FORMULAIRE))) {
        ...
        if (isset($_GET[BAZ_VARIABLE_VOIR]) && $_GET[BAZ_VARIABLE_VOIR] == BAZ_VOIR_FORMULAIRE) {
            $class = ' btn-primary';
        } else {
            $class = ' btn-default';
        }
    ...
    if (strstr(BAZ_VOIR_AFFICHER, strval(BAZ_VOIR_LISTES))) {
        ...
        if (isset($_GET[BAZ_VARIABLE_VOIR]) && $_GET[BAZ_VARIABLE_VOIR] == BAZ_VOIR_LISTES) {
            $class = ' btn-primary';
        } else {
            $class = ' btn-default';
        }
    ...
    if (strstr(BAZ_VOIR_AFFICHER, strval(BAZ_VOIR_IMPORTER))) {
        ...
        if (isset($_GET[BAZ_VARIABLE_VOIR]) && $_GET[BAZ_VARIABLE_VOIR] == BAZ_VOIR_IMPORTER) {
            $class = ' btn-primary';
        } else {
            $class = ' btn-default';
        }
    ...
    if (strstr(BAZ_VOIR_AFFICHER, strval(BAZ_VOIR_EXPORTER))) {
        ...
        if (isset($_GET[BAZ_VARIABLE_VOIR]) && $_GET[BAZ_VARIABLE_VOIR] == BAZ_VOIR_EXPORTER) {
            $class = ' btn-primary';
        } else {
            $class = ' btn-default';
        }
...
function baz_afficher_formulaire_import()
{
    ...
    if ($GLOBALS['wiki']->UserIsAdmin()) {
        ...
        if (!isset($categorienature)) {
            $categorienature = 'toutes';
        }
        $id_type_fiche = isset($_REQUEST['id_type_fiche']) ? $_REQUEST['id_type_fiche'] : '';
        ...
        if (isset($_POST['submit_file'])) {
            ...
            foreach ($tableau as $ligne) {
                if ($ligne[0] != 'labelhtml') {
                    if ($ligne[0] == 'liste' || $ligne[0] == 'checkbox' ||
                        $ligne[0] == 'listefiche' || $ligne[0] == 'checkboxfiche') {
                        $nom_champ[] = $ligne[0].$ligne[1].$ligne[6];
                        $type_champ[] = $ligne[0];
                        $idliste_champ[$ligne[0].$ligne[1].$ligne[6]] = $ligne[1];
                ...
                if ($ext == 'csv') {
                            ...
                            if ($row > 1) {
                                for ($c = 0; $c < $num; $c++) {
                                    if (TEMPLATES_DEFAULT_CHARSET != 'UTF-8') {
                                        $valeur[$nom_champ[$c]] = utf8_decode($data[$c], ENT_QUOTES, 'ISO-8859-15');
                                    } else {
                                    ...
                                    if ($nom_champ[$c] == 'bf_latitude' && !empty($data[$c])) {
                                        $bf_latitude = $data[$c];
                                        $geolocalisation = true;
                                    }
                                    if ($nom_champ[$c] == 'bf_longitude' && !empty($data[$c])) {
                                        $bf_longitude = $data[$c];
                                        $geolocalisation = true;
                                    }

                                    // recuperer les id pour les listes et checkbox plutot que leur labels
                                    if (($type_champ[$c] == 'checkbox' || $type_champ[$c] == 'liste') &&
                                                isset($data[$c]) && !empty($data[$c])) {
                                        if ($type_champ[$c] == 'liste') {
                                            $idval = array_search($data[$c], $alllists[$idliste_champ[$nom_champ[$c]]]['label']);
                                        } elseif ($type_champ[$c] == 'checkbox') {
                                            $tab_chkb = explode(',', $data[$c]);
                                            $tab_chkb = array_map('trim', $tab_chkb);
                                            $tab_id = array();
                                            foreach ($tab_chkb as $value) {
                                                $tab_id[] = array_search($value, $alllists[$idliste_champ[$nom_champ[$c]]]['label']);
                                            }
                                    ...
                                    if (($type_champ[$c]) == 'image' && isset($data[$c]) && !empty($data[$c])) {
                                        $imageorig = trim(utf8_decode($data[$c]));
                                        ...
                                        if (file_exists(BAZ_CHEMIN_UPLOAD.$imageorig)) {
                                            ...
                                            $errormsg[] = _t('BAZ_IMAGE_FILE_NOT_FOUND').' : '.$imageorig;
                                            $erreur = true;
                                        }
                                    }
                                ...
                                if (count($errormsg) > 0) {
                                    $outputerror .= '<label>
                                            <input type="checkbox" disabled> '.$valeur['bf_titre'].'
                                            </label>
                                            <a class="btn-mini btn-xs btn btn-default" data-target="#collapse'.$valeur['id_fiche'].'" data-toggle="collapse"><i class="glyphicon glyphicon-eye-open icon-eye-open icon-white"></i> '._t('BAZ_SEE_ENTRY').'</a>
                                <div class="panel panel-danger">
                                    <div id="collapse'.$valeur['id_fiche'].'" class="panel-collapse collapse">
                                      <div class="panel-body">
                                        <div class="alert alert-danger">'.implode('<br>', $errormsg).'</div>'.
                                    ...
                                    $outputright .= '<label>
                                            <input type="checkbox" name="importfiche['.$valeur['id_fiche'].']" value=\''.base64_encode(serialize($valeur)).'\'> '.$valeur['bf_titre'].'
                                            </label>
                                            <a class="btn-mini btn-xs btn btn-default" data-target="#collapse'.$valeur['id_fiche'].'" data-toggle="collapse"><i class="glyphicon glyphicon-eye-open icon-eye-open icon-white"></i> '._t('BAZ_SEE_ENTRY').'</a>
                    ...
                    $output .= '<div class="checkbox">
                                <label class="checkbox">
                                    <input data-target="#accordion-import" type="checkbox" class="selectall"> '._t('BAZ_SELECT_ALL').'
                                </label>
                            </div>
                            ...
                            '<input type="hidden" value="'.$id_type_fiche.'" name="id_type_fiche" />'."\n".
                            '<button class="btn btn-primary" type="submit">'._t('BAZ_IMPORT_SELECTION').'</button>'."\n";
                }
            }
            ...
            $GLOBALS['_BAZAR_']['id_typeannonce'] = $id_type_fiche;
            $GLOBALS['_BAZAR_']['categorie_nature'] = $val_formulaire['bn_type_fiche'];
            $GLOBALS['_BAZAR_']['template'] = $val_formulaire['bn_template'];
            $importlist = '';
            $nb = 0;
            foreach ($_POST['importfiche'] as $WikiName => $valeur) {
                ...
                $nb++;
                $importlist .= ' '.$nb.') [['.$valeur['id_fiche'].' '.$valeur['bf_titre'].']]'."\n";
            }
...
function baz_formulaire($mode, $url = '', $valeurs = '')
{
    ...
    $squelette->setElementTemplate('
        <div class="form-actions form-group"><div class="col-sm-9 col-sm-offset-3">{label}{element}</div></div>'."\n", 'groupe_boutons');
            ...
            if (count($resultat) == 0) {
                ...
                foreach ($resultat as $ligne) {
                    if ($ligne['bn_image_titre'] != '') {
                        ...
                        $titre = '<strong>'.$ligne['bn_label_nature'].((empty($ligne['bn_description'])) ? '' : ' :<br>').'</strong>'."\n".$ligne['bn_description']."\n";
                    }
                    $radiohtml = '<div class="radio">
                    <label><input id="select'.$ligne['bn_id_nature'].'" name="id_typeannonce" value="'.$ligne['bn_id_nature'].'" type="radio">'.$titre.'</label></div>'."\n".'<br>'."\n";
                ...
                $buttons[] = $HTML_QuickForm->createElement('link', 'annuler', _t('BAZ_ANNULER'), str_replace('&amp;', '&', $GLOBALS['_BAZAR_']['url']->getURL()), _t('BAZ_ANNULER'), array('class' => 'btn btn-mini btn-xs btn-danger bouton_annuler'));
                $formtemplate->addGroup($buttons, 'groupe_boutons', null, '&nbsp;', 0);
                $squelette->setElementTemplate('<div class="control-group form-group">'."\n".'<div class="radio"> '."\n".'{element}'."\n".'<!-- BEGIN error --><span class="alert alert-danger">{error}</span><!-- END error -->'."\n".'</div>'."\n".'</div>'."\n");
...
function baz_afficher_formulaire_fiche($mode = 'saisie', $formtemplate, $url = '', $valeurs = '')
{
    ...
    if ($GLOBALS['_BAZAR_']['condition'] != '' && !isset($_POST['accept_condition']) && !isset($_POST['bf_titre'])) {
        ...
        $buttons[] = &HTML_QuickForm::createElement('link', 'annuler', _t('BAZ_ANNULER'), str_replace('&amp;', '&', ($url ? str_replace('/edit', '', $url) : $GLOBALS['_BAZAR_']['url']->getURL())), _t('BAZ_ANNULER'), array('class' => 'btn btn-mini btn-xs btn-danger bouton_annuler'));
        $formtemplate->addGroup($buttons, 'groupe_boutons', null, '&nbsp;', 0);
    }
...
function baz_requete_bazar_fiche($valeur)
{
    ...
    if (isset($valeur['sendmail'])) {
        if ($valeur[$valeur['sendmail']] != '') {
            $destmail = $valeur[$valeur['sendmail']];
        }
...
function baz_insertion_fiche($valeur, $batch = false)
{
    ...
    if (isset($valeur['bf_titre'])) {
        ...
        if (isset($GLOBALS['utilisateur_wikini'])) {
            ...
            $GLOBALS['_BAZAR_']['nomwiki'] = $GLOBALS['wiki']->LoadUser($GLOBALS['utilisateur_wikini']);
            $GLOBALS['wiki']->SetUser($GLOBALS['_BAZAR_']['nomwiki']);
        }

        //createur de la fiche
        if (isset($GLOBALS['_BAZAR_']['nomwiki']) && !empty($GLOBALS['_BAZAR_']['nomwiki'])) {
            $valeur['createur'] = $GLOBALS['_BAZAR_']['nomwiki']['name'];
        } else {
            $valeur['createur'] = _t('BAZ_ANONYME');
        }
        ...
        $saved = $GLOBALS['wiki']->SavePage($GLOBALS['_BAZAR_']['id_fiche'], json_encode($valeur));
        ...
        if (isset($GLOBALS['utilisateur_wikini'])) {
            ...
            if (!empty($olduser)) {
                ...
                $GLOBALS['_BAZAR_']['nomwiki'] = $olduser;
            }
        }
...
function baz_formulaire_des_formulaires($mode, $valeursformulaire = '')
{
    ...
    $squelette->setElementTemplate('<div class="form-actions form-group"><div class="col-sm-9 col-sm-offset-3">{label}{element}</div></div>'."\n", 'groupe_boutons');
    $squelette->setRequiredNoteTemplate("\n".'<div class="col-sm-9 col-sm-offset-3 symbole_obligatoire">* {requiredNote}</div>'."\n");
    ...
    if ($idformulaire != '') {
        if (!isset($GLOBALS['bazar']['form'][$idformulaire])) {
            ...
            foreach ($tab_resultat as $key => $value) {
                $GLOBALS['bazar']['form'][$idformulaire][$key] = _convert($value, 'ISO-8859-15');
            }
        }
        $GLOBALS['bazar']['form'][$idformulaire]['template'] = formulaire_valeurs_template_champs(
            $GLOBALS['bazar']['form'][$idformulaire]['bn_template']
        );

        return $GLOBALS['bazar']['form'][$idformulaire];
    } else {
        $requete = 'SELECT bn_id_nature FROM '.BAZ_PREFIXE.'nature ORDER BY bn_type_fiche ASC, bn_label_nature ASC';
        $tab_resultat = $GLOBALS['wiki']->LoadAll($requete);
        foreach ($tab_resultat as $key => $value) {
            $GLOBALS['bazar']['form'][$value['bn_id_nature']] = baz_valeurs_formulaire($value['bn_id_nature']);
        }
...
function baz_formulaire_des_listes($mode, $valeursliste = '')
{
    ...
    $GLOBALS['wiki']->addJavascriptFile('tools/bazar/libs/vendor/jquery-ui-sortable/jquery-ui-1.9.1.custom.min.js');
    ...
    if (!is_file($templatetoload)) {
        $templatetoload = 'tools/bazar/presentation/templates/lists_edit.tpl.html';
    }
...
function baz_gestion_formulaire()
{
    ...
    if ($GLOBALS['wiki']->GetUser()) {
        ...
        if (!isset($_GET['action_formulaire']) ||
            ($_GET['action_formulaire'] != 'modif' && $_GET['action_formulaire'] != 'new')) {
...
function baz_gestion_listes()
{
    ...
    if (!isset($_GET['action'])) {
        ...
        foreach ($resultat as $ligne) {
            ...
            $tab_lists['lists'][$ligne['resource']]['can_delete'] = $GLOBALS['wiki']->UserIsAdmin() ||
            ...
            if ($elements_liste != '') {
                $tab_lists['lists'][$ligne['resource']]['values'] = '<select class="form-control" id="liste_'.$ligne['resource'].'">'."\n".'<option>'._t('BAZ_CHOISIR').'</option>'."\n".$elements_liste.'</select>'."\n";
            } else {
    ...
    if ($idfiche != '') {
        ...
        if ($type_page == 'fiche_bazar') {
            ...
            if (!isset($valeurs_fiche['id_typeannonce'])) {
                $valeurs_fiche['id_typeannonce'] = $valeurs_fiche['id_typeannonce'];
            }
    ...
    if ($idliste != '') {
        if (!isset($GLOBALS['bazar']['lists'][$idliste])) {
            ...
            if ($GLOBALS['wiki']->GetTripleValue($idliste, 'http://outils-reseaux.org/_vocabulary/type', '', '') == 'liste') {
                ...
                if (TEMPLATES_DEFAULT_CHARSET != 'UTF-8') {
                    $GLOBALS['bazar']['lists'][$idliste]['titre_liste'] = utf8_decode($valeurs_fiche['titre_liste']);
                    $GLOBALS['bazar']['lists'][$idliste]['label'] = array_map('utf8_decode', $valeurs_fiche['label']);
                } else {
        ...
        foreach ($resultat as $ligne) {
            $GLOBALS['bazar']['lists'][$ligne['resource']] = baz_valeurs_liste($ligne['resource']);
        }
...
function baz_rechercher($typeannonce = 'toutes', $categorienature = 'toutes')
{
    ...
    if (isset($_REQUEST['id_typeannonce']) && $_REQUEST['id_typeannonce'] != 'toutes' && $_REQUEST['id_typeannonce'] != '') {
        ...
        for ($i = 0; $i < count($tableau); $i++) {
            if (isset($tableau[$i][0]) && ($tableau[$i][0] == 'liste' || $tableau[$i][0] == 'checkbox' || $tableau[$i][0] == 'listefiche' || $tableau[$i][0] == 'checkboxfiche' || $tableau[$i][0] == 'labelhtml')) {
...
function baz_requete_recherche_fiches($tableau_criteres = '', $tri = '', $id_typeannonce = '', $categorie_fiche = '', $statut = 1, $personne = '', $nb_limite = '', $motcles = true, $searchstring = '', $facettesearch = 'OR')
{
    ...
    if ($id_typeannonce == '' && isset($GLOBALS['_BAZAR_']['id_typeannonce'])) {
        $id_typeannonce = $GLOBALS['_BAZAR_']['id_typeannonce'];
    }
    if ($categorie_fiche == '' && isset($GLOBALS['_BAZAR_']['categorie_nature'])) {
        $categorie_fiche = $GLOBALS['_BAZAR_']['categorie_nature'];
    }

    //requete pour recuperer toutes les PageWiki etant des fiches bazar
    $requete_pages_wiki_bazar_fiches = 'SELECT DISTINCT resource FROM '.BAZ_PREFIXE.'triples '.
