...
function baz_afficher_liste_fiches_utilisateur()
{
    ...
    if (baz_a_le_droit('voir_mes_fiches') && isset($nomwiki['name'])) {
        $tableau_dernieres_fiches = $GLOBALS['wiki']->services->get('bazar.fiche.manager')->search([ 'formsIds'=>$GLOBALS['params']['idtypeannonce'], 'user'=>$nomwiki['name'] ]);
        ...
        if (isset($_POST['submit_file'])) {
                                    ...
                                    if (isset($nom_champ[$c])) {
                                        ...
                                        if (($type_champ[$c] == 'checkboxfiche' || $type_champ[$c] == 'listefiche') &&
                                            ...
                                            if (!isset($allentries[$idfiche])) {
                                                $fa = $GLOBALS['wiki']->services->get('bazar.fiche.manager')->search();
                                                $tabfa = array();
                                                foreach ($fa as $fares) {
            ...
            if (!isset($GLOBALS['importdone'])) {
                ...
                foreach ($_POST['importfiche'] as $fiche) {
                    ...
                    $fiche['antispam'] = 1;
                    $fiche = $GLOBALS['wiki']->services->get('bazar.fiche.manager')->create($id, $fiche);
...
function baz_afficher_formulaire_export()
{
    ...
    if ($GLOBALS['wiki']->UserIsAdmin()) {
        ...
        $tableau_fiches = $GLOBALS['wiki']->services->get('bazar.fiche.manager')->search([ 'queries'=>$query, 'formsIds'=>[$id], 'keywords' => $q ]);
...
function baz_formulaire($mode, $url = '', $valeurs = '')
{
    ...
    if ($mode == BAZ_ACTION_NOUVEAU_V) {
        try {
            $fiche = $GLOBALS['wiki']->services->get('bazar.fiche.manager')->create($_POST['id_typeannonce'], $_POST);
    ...
    if ($mode == BAZ_ACTION_MODIFIER_V) {
        try {
            $fiche = $GLOBALS['wiki']->services->get('bazar.fiche.manager')->update($_POST['id_fiche'], $_POST);
  ...
  <label>
    ...
    '._t('BAZ_ACCEPTE_CONDITIONS').'
        ...
        if (!is_array($valeurs) && !empty($valeurs) && $GLOBALS['wiki']->isWikiName($valeurs)) {
            ...
            $valeurs = $GLOBALS['wiki']->services->get('bazar.fiche.manager')->getOne($valeurs);
        }
        $data['content'] = '';
        $formtemplate = '';
        ...
        if (in_array($formelem[0], array('radio', 'liste', 'checkbox', 'listefiche', 'checkboxfiche'))) {
            ...
            if (in_array($formelem[0], array('radio', 'liste', 'checkbox'))) {
                ...
                if (!isset($result[$hash])) {
                    $result[$hash] = $GLOBALS['wiki']->services->get('bazar.fiche.manager')->search([
                        'queries'=>$tabquery,
                        'formsIds'=>$formelem[1],
...
function baz_voir_fiche($danslappli, $idfiche, $form = '')
{
    ...
    if (is_array($idfiche)) {
        ...
        $fichebazar['values'] = $GLOBALS['wiki']->services->get('bazar.fiche.manager')->getOne($idfiche);
    ...
    if ($custom_template) {
        ...
        // } else {
            ...
            try {
                $html['semantic'] = $GLOBALS['wiki']->services->get('bazar.fiche.manager')->convertToSemanticData($fichebazar['form']['bn_id_nature'], $html, true);
            } catch(\Exception $e) {
                ...
                if ($facettevalue[$id]['type'] == 'liste') {
                    ...
                    foreach ($facettevalue[$id] as $idfiche => $nb) {
                        if ($idfiche != 'source' && $idfiche != 'type') {
                            $f = $GLOBALS['wiki']->services->get('bazar.fiche.manager')->getOne($idfiche);
...
function baz_afficher_flux_RSS()
{
    ...
    $tableau_flux_rss = $GLOBALS['wiki']->services->get('bazar.fiche.manager')->search([
        'queries'=>$query,
        'formsIds'=>$id,
        'user'=>$utilisateur,
        'keywords'=>$q
