...
function baz_mise_a_jour_fiche($valeur) {
	echo '<pre>VALEUR MERGE:<br />';
	var_dump(array_merge(baz_valeurs_fiche($GLOBALS['_BAZAR_']['id_fiche']), $valeur));
	echo '<br /><br />VALEUR INTER:<br/>';
	...
	//il y a des donnees pour ajouter une nouvelle liste
		...
		foreach ($_POST["label"] as $label) {
			if (($label!=NULL || $label!='') && ($_POST["id"][$i]!=NULL || $_POST["id"][$i]!='')) {
				$valeur["label"][htmlentities($_POST["id"][$i])] = htmlentities($label);
				$i++;
			}
		}
		$valeur["label"] = array_map("utf8_encode", $valeur["label"]);
		$valeur["titre_liste"] = utf8_encode(htmlentities($_POST["titre_liste"]));
