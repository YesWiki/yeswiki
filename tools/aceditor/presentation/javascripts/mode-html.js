...
ace.define('ace/mode/css_highlight_rules', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/lib/lang', 'ace/mode/text_highlight_rules'], (require, exports, module) => {
  const oop = require('../lib/oop')
...
ace.define('ace/mode/css_completions', ['require', 'exports', 'module'], (require, exports, module) => {
  const propertyMap = {
    background: { '#$0': 1 },
  ...
  (function() {
    ...
    this.defineCompletions = function() {
      if (document) {
        const { style } = document.createElement('c')

        for (const i in style) {
          if (typeof style[i] !== 'string') { continue }

          const name = i.replace(/[A-Z]/g, (x) => `-${x.toLowerCase()}`)
...
ace.define('ace/mode/behaviour/css', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/mode/behaviour', 'ace/mode/behaviour/cstyle', 'ace/token_iterator'], (require, exports, module) => {
  const oop = require('../../lib/oop')
  const { Behaviour } = require('../behaviour')
  const { CstyleBehaviour } = require('./cstyle')
  const { TokenIterator } = require('../../token_iterator')

  const CssBehaviour = function() {
    this.inherit(CstyleBehaviour)

    this.add('colon', 'insertion', (state, action, editor, session, text) => {
    ...
    this.add('colon', 'deletion', (state, action, editor, session, range) => {
    ...
    this.add('semicolon', 'insertion', (state, action, editor, session, text) => {
      if (text === ';' && editor.selection.isEmpty()) {
        const cursor = editor.getCursorPosition()
        const line = session.doc.getLine(cursor.row)
        const rightChar = line.substring(cursor.column, cursor.column + 1)
    ...
    this.add('!important', 'insertion', (state, action, editor, session, text) => {
      if (text === '!' && editor.selection.isEmpty()) {
        const cursor = editor.getCursorPosition()
        const line = session.doc.getLine(cursor.row)
...
ace.define('ace/mode/css', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/mode/text', 'ace/mode/css_highlight_rules', 'ace/mode/matching_brace_outdent', 'ace/worker/worker_client', 'ace/mode/css_completions', 'ace/mode/behaviour/css', 'ace/mode/folding/cstyle'], (require, exports, module) => {
  const oop = require('../lib/oop')
  const TextMode = require('./text').Mode
  const { CssHighlightRules } = require('./css_highlight_rules')
  const { MatchingBraceOutdent } = require('./matching_brace_outdent')
  const { WorkerClient } = require('../worker/worker_client')
  const { CssCompletions } = require('./css_completions')
  const { CssBehaviour } = require('./behaviour/css')
  const CStyleFoldMode = require('./folding/cstyle').FoldMode
...
ace.define('ace/mode/html_highlight_rules', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/lib/lang', 'ace/mode/css_highlight_rules', 'ace/mode/javascript_highlight_rules', 'ace/mode/xml_highlight_rules'], (require, exports, module) => {
  const oop = require('../lib/oop')
  const lang = require('../lib/lang')
  const { CssHighlightRules } = require('./css_highlight_rules')
  const { JavaScriptHighlightRules } = require('./javascript_highlight_rules')
  const { XmlHighlightRules } = require('./xml_highlight_rules')
    ...
    this.add('autoclosing', 'insertion', function(state, action, editor, session, text) {
      if (text == '>') {
        ...
        if (is(token, 'attribute-value')) {
          const tokenEndColumn = iterator.getCurrentTokenColumn() + token.value.length
...
ace.define('ace/mode/folding/mixed', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/mode/folding/fold_mode'], (require, exports, module) => {
  const oop = require('../../lib/oop')
  const BaseFoldMode = require('./fold_mode').FoldMode

  const FoldMode = exports.FoldMode = function(defaultMode, subModes) {
  ...
  (function() {
    ...
    this._pop = function(stack, tag) {
      while (stack.length) {
        ...
        if (this.optionalEndTags.hasOwnProperty(top.tagName)) {
          stack.pop()
          continue
        } else {
          return null
...
ace.define('ace/mode/folding/html', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/mode/folding/mixed', 'ace/mode/folding/xml', 'ace/mode/folding/cstyle'], (require, exports, module) => {
  const oop = require('../../lib/oop')
  const MixedFoldMode = require('./mixed').FoldMode
  const XmlFoldMode = require('./xml').FoldMode
  const CStyleFoldMode = require('./cstyle').FoldMode

  const FoldMode = exports.FoldMode = function(voidElements, optionalTags) {
...
ace.define('ace/mode/html_completions', ['require', 'exports', 'module', 'ace/token_iterator'], (require, exports, module) => {
  const { TokenIterator } = require('../token_iterator')
  ...
  const HtmlCompletions = function() {
...
ace.define('ace/mode/html', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/lib/lang', 'ace/mode/text', 'ace/mode/javascript', 'ace/mode/css', 'ace/mode/html_highlight_rules', 'ace/mode/behaviour/xml', 'ace/mode/folding/html', 'ace/mode/html_completions', 'ace/worker/worker_client'], (require, exports, module) => {
  const oop = require('../lib/oop')
  const lang = require('../lib/lang')
  const TextMode = require('./text').Mode
  const JavaScriptMode = require('./javascript').Mode
  const CssMode = require('./css').Mode
  const { HtmlHighlightRules } = require('./html_highlight_rules')
  const { XmlBehaviour } = require('./behaviour/xml')
  const HtmlFoldMode = require('./folding/html').FoldMode
  ...
  (function() {
    ...
    this.createWorker = function(session) {
      ...
      const worker = new WorkerClient(['ace'], 'ace/mode/html_worker', 'Worker')
      worker.attachToDocument(session.getDocument())
