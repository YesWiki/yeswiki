...
export default function createAceditorToolbar(textarea, aceditor, options = {}) {
  const toolbar = $('<div>').addClass('btn-toolbar aceditor-toolbar')

  const syntax = SYNTAX[options.syntax || 'yeswiki']
  ...
  let actionsHtml = ''
  for (const actionGroupName in actionsBuilderData.action_groups) {
    const groupConfig = actionsBuilderData.action_groups[actionGroupName]
