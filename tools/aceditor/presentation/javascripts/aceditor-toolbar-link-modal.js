export default function($button, aceditor) {
  /* get pageList */
  if (!pagelist) {
    var pagelist = []
    $.ajax({
  ...
  const $linkmodal = $('#YesWikiLinkModal')

  $('.btn-insert').on('click', (e) => {
    const $inputUrl = $linkmodal.find('[name="wikiurl"]')
    let wikiurl = $inputUrl.val() || ''
    ...
    // Create wiki code
    const text = $linkmodal.find('[name="text-url"]').val() || wikiurl
    const linkOption = $linkmodal.find('.link-options').val()
    if (linkOption == 'link') {
