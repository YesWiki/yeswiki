...
if (!$incPage = $this->LoadPage($incPageName)) {
	...
	$plugin_output_new = '<a class="yeswiki-editable" href="'.$this->href('edit', $incPageName, $query_string).'"><i class="icon-pencil"></i>&nbsp;Editer '.$incPageName.'</a>';
}
...
if (!empty($actif) && $actif=="1") {
       ...
       $plugin_output_new=str_ireplacement('<a href="'.$this->config["base_url"].$page_active.'"','<a class="active-link" href="'.$this->config["base_url"].$page_active.'"', $plugin_output_new);
}
