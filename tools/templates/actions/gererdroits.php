...
foreach ($liste_themes_templates as $theme => $valeur) {
        ?>
		<option value="<?php echo $theme;?>" <?php if ($theme == $theme_choisi) {echo "selected";}
...
<?php
	...
	<input name="choixtheme" type="submit" class="btn <?php if ($btnclass != '') {
        echo ' ' . $btnclass;
    }
    ?>" value="S&eacute;lection du th&egrave;me"/><br>
...
foreach ($liste_themes_templates[$theme_choisi]['squelette'] as $squelette) {
            ?>
				<option value="<?php echo $squelette;?>" <?php if ($squelette == $squelette_choisi) {echo "selected";}
            ?>><?php echo $squelette;?></option>
<?php
}
        ?>
			</select>
			<br>
			<input name="choixsquelette" type="submit" class="btn <?php if ($btnclass != '') {
            echo ' ' . $btnclass;
        }
        ?>" value="S&eacute;lection du squelette"/><br>
<?php

        if ($squelette_choisi == "tous_les_squelettes") {
            $num_page = 0;
            while ($tab_liste_pages = mysqli_fetch_array($liste_pages)) {
                $table_triples = mysqli_fetch_array($this->Query("SELECT * FROM " . $table . "triples WHERE resource='" . $tab_liste_pages["tag"] . "'"));
...
<?php
	...
	<table class="table">
		...
		<tr>
			<td><textarea name="newlire" rows=4 cols=10 ><?php if (isset($_POST["newlire"])) {
        echo $_POST["newlire"];
    }
    ?></textarea></td>
			<td><textarea name="newecrire" rows=4 cols=10 ><?php if (isset($_POST["newecrire"])) {
        echo $_POST["newecrire"];
    }
    ?></textarea></td>
			<td><textarea name="newcomment" rows=4 cols=10 ><?php if (isset($_POST["newcomment"])) {
