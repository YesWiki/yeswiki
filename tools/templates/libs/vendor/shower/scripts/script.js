...
$('.attached_file').each(function() {
	var $fig = $(this);
	$fig.parent().find('header').after($fig.find('img').removeAttr('width').removeAttr('height'));

});
window.shower = (function(window, document, undefined) {
	var shower = {},
		url = window.location,
		initialsearch = url.search.replace('&full', ''),
		body = document.body,
		slides = document.querySelectorAll('.slide'),
		progress = document.querySelector('div.slide-progress div'),
		slideList = [],
		timer,
	...
	shower._dispatchSingleSlideMode = function(e) {
		...
		if ('' !== slideId && shower.isListMode()) {
			...
			if (isHistoryApiSupported) {
				if (initialsearch != '') {
					history.replaceState(null, null, url.pathname + initialsearch + '&full#' + slideId);
				}
				else {
	...
	window.addEventListener('DOMContentLoaded', function () {
		if ( ! shower.isListMode()) {
			...
			if (-1 === shower.getCurrentSlideNumber()) {
				if (isHistoryApiSupported) {
					if (initialsearch != '') {
						history.replaceState(null, null, url.pathname + initialsearch + '&full' + shower.getSlideHash(0));
					}
					else {
		...
		switch (e.which) {
			...
			case 13: // Enter
				if (shower.isListMode() && -1 !== currentSlideNumber) {
					...
					if (isHistoryApiSupported) {
						if (initialsearch != '') {
							history.replaceState(null, null, url.pathname + initialsearch + '&full' + shower.getSlideHash(currentSlideNumber));
						}
						else {
							history.replaceState(null, null, url.pathname + '?full' + shower.getSlideHash(currentSlideNumber));
						}
					}
					shower.enterSlideMode();
			...
			case 27: // Esc
				if ( ! shower.isListMode()) {
					...
					if (isHistoryApiSupported) {
						if (initialsearch != '') {
							history.replaceState(null, null, url.pathname + initialsearch + shower.getSlideHash(currentSlideNumber));
						}
						else {
							history.replaceState(null, null, url.pathname + shower.getSlideHash(currentSlideNumber));
						}
					}
					shower.enterListMode();
