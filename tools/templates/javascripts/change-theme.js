;(function ($) {
      ...
      if (element[0]) {
        ...
        if (type === 'preset') {
          ...
          const anchor = Object.values(currentList)
            .map((val) => val.replace(/(\.css|\.tpl.html)$/, ''))
            .includes(curVal)
            ? curVal
            : favorite
          Object.keys(currentList).forEach((k) => {
            ...
            element[0].options.add(
              new Option(shortValue, k, false, anchor === k),
            )
          })
        }
      }
  ...
  const extractData = (currentBase) => {
    ...
    favorites =
      typeof favorites === 'object' && favorites !== null ? favorites : {}
    ...
    updateUrl =
      ['string', 'boolean'].includes(typeof updateUrl) &&
      [true, 'true'].includes(updateUrl)
