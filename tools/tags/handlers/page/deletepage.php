...
{
	if ($_GET['eraselink'] == 'oui') {
		$this->Query("DELETE FROM ".$this->config["table_prefix"]."links WHERE to_tag = '".$this->GetPageTag()."'");
	}			
	if ($this->IsOrphanedPage($this->GetPageTag()))
	{
		...
		else
		{
			...
			$this->Query("DELETE FROM ".$this->config["table_prefix"]."triples WHERE resource = '".$this->GetPageTag()."'");
	...
	else
	{
		...
		$msg .= '" method="post" style="display: inline">'."\n";
		$msg .= 'Voulez-vous vraiment supprimer d&eacute;finitivement la page malgr&eacute; la pr&eacute;sence de liens ? '."\n";
		$msg .= '<input type="submit" value="Oui" ';
		$msg .= 'style="vertical-align: middle; display: inline" />'."\n";
		$msg .= "</form>\n";			
		$msg .= '<form action="'.$this->Href().'" method="post" style="display: inline">'."\n";
