...
class HashcashHandler extends YesWikiHandler
{
    public function run()
    {
        $hashcashService = $this->getService(HashCashService::class);
        ...
        $function_name = $hashcashService->hashcash_random_string(rand(6, 18));
        ...
        switch ($type) {
            ...
            case 0:
                $eax = $hashcashService->hashcash_random_string(rand(8, 10), $expired);
                $expired [] = $eax;

                $val = intval($hashcashService->hashcash_field_value());
            ...
            case 1:
                ...
                $ebx = $hashcashService->hashcash_random_string(rand(8, 10), $expired);
                $expired [] = $ebx;

                $ecx = $hashcashService->hashcash_random_string(rand(8, 10), $expired);
                $expired [] = $ecx;

                $val = $hashcashService->hashcash_field_value();
        ...
        $libs_name = $hashcashService->hashcash_random_string(rand(6, 18), $expired);
        $expired [] = $libs_name;

        $libs = "function $libs_name(){";

        // write bytes to javascript, xor with key
        $data_name = $hashcashService->hashcash_random_string(rand(6, 18), $expired);
        $expired [] = $data_name;
