...
if ($this->UserIsInGroup('admins'))
{
	...
	if (isset($tabnature[0])) {
		...
		foreach ($tab as $ligne) {
			if ($ligne['bl_label_liste']!=$anciennomliste) {			
				...
				$nomwikiliste = genere_nom_wiki(html_entity_decode('Liste '.$ligne['bl_label_liste']),0);
	...
	if (isset($tabnature[0])) {
		...
		foreach ($resultat as $ligne) {
			$nomwikiformulaire = genere_nom_wiki(html_entity_decode('Formulaire '.$ligne['bn_label_nature']), 0);
