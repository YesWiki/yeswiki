...
if (!function_exists('tronqueHTML')) {
function tronqueHTML($text, $length = 100, $ending = '...', $exact = true, $considerHtml = true) {
	if ($considerHtml) {
	...
	/*
	function tronqueHTML($text, $length = 100, $ending = '...', $exact = true, $considerHtml = false) {
		if ($considerHtml) {
			// if the plain text is shorter than the maximum length, return the whole text
			if (strlen(preg_replace('/<.*?>/', '', $text)) <= $length) {
				return $text;
					...
					foreach ($feed->items as $item) {					
						...
						if (!empty($nbchar)) {
							...
							if (strlen($item['description']) > 0 &&  strlen($item['description']) > $nbchar) {
							    ...
							    $item['description'] = cut_html_string($item['description'], $nbchar).' [..] <a class="lien_lire_suite" href="'.$aso_page['url'].'" '.
							    //on ouvre le lien lire la suite dans une nouvelle fenetre, ou pas.
