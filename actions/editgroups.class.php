...
class ActionEditgroups extends WikiniAdminAction
{
	function PerformAction($args)
	{
		...
		{
			$res .= '<option value="' . htmlspecialchars($group, ENT_COMPAT | ENT_HTML401, TEMPLATES_DEFAULT_CHARSET) . '">' . htmlspecialchars($group, ENT_COMPAT | ENT_HTML401, TEMPLATES_DEFAULT_CHARSET) .  '</option>';
		}
		...
		elseif (!empty($_GET['groupname']))
		{
			...
			{
				$res .= $wiki->FormOpen();
				$res .= '<br />&Eacute;diter le groupe <strong>' . htmlspecialchars($name, ENT_COMPAT | ENT_HTML401, TEMPLATES_DEFAULT_CHARSET) . '</strong>: <br />';
