...
if ($this->GetMethod() != 'show' || empty($this->page)) {
    ...
    echo '  <meta property="og:site_name" content="'
        . $this->config['yeswiki_name'] . '" />' . "\n";
    $utils = $this->services->get(TemplateHelperService::class);
    ...
    $h = 630; // image height
    if (!empty($this->page)) {
        $img = $this->services->get(TemplateHelperService::class)->getImageFromBody($this->page, strval($w), strval($h));
    }
    if (!empty($img)) {
