...
class LinkTracker
{
    ...
    {
        if (preg_match_all('/{{(?:adminthemes|setwikidefaulttheme|admintag|editgroups|userstable|editconfig|adminacls|update\\s*(?:version="[^"]*")?|syndication(?:[^}]|\\s)*|listpages(?:[^}]|\\s)*|entrylist(?:[^}]|\\s)*|entrymap(?:[^}]|\\s)*|bazar(?:[^}]|\\s)*)\\s*}}/i', $body, $matches)) {
            foreach ($matches[0] as $key => $value) {
                $body = str_replace($matches[0][$key], '', $body);
            }
        }
