...
class EditActionsAclsAction extends YesWikiAction implements RegisteredAction
{
    ...
    public function run()
    {
        ...
        $wiki = $this->services;
        $list = $wiki->get(Performer::class)->list('action');
        sort($list);
        ...
        $res .= '</select> <input type="submit" class="btn btn-default" value="' . _t('SEE') . '" />' . $wiki->get(\YesWiki\Render\Service\TemplateEngine::class)->formClose();
