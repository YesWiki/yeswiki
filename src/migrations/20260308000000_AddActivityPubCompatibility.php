...
class AddActivityPubCompatibility extends YesWikiMigration
{
    public function run()
    {
        if (!$this->dbService->schema()->columnExists('nature', 'bn_activitypub_enable')) {
        ...
        $this->dbService->schema()->dropColumn('nature', 'bn_sem_context');
        $this->dbService->schema()->dropColumn('nature', 'bn_sem_type');
        $this->dbService->schema()->dropColumn('nature', 'bn_sem_use_template');
    }
}
