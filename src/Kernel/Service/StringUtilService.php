...
{
    /** Percent-encode every byte outside printable ASCII, which is what a browser puts on the wire, so an address carrying accents or emoji keeps its structure. */
    public static function encodeUrlNonAscii(string $url): string
    {
        ...
        return (string)preg_replace_callback('/[^\x20-\x7E]/', function ($match) {
            return rawurlencode($match[0]);
        }, self::encodeUrlHost($url));
    }

    /** A host carrying accents is spelled in punycode, not percent-encoded, so it has to be converted before the rest of the address is encoded byte by byte. */
    ...
    /** Judge an address on its structure alone, since FILTER_VALIDATE_URL refuses accents and emoji that browsers and sites like LinkedIn accept. */
