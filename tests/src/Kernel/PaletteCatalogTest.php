...
/** Ticket 36 folded `docs/actions/lang/` into the main catalog. */
class PaletteCatalogTest extends TestCase
{
    /**
     * The one place a palette `_t()` key is not a literal: `EntryListAction` names a webmaster's own `custom/templates/bazar/*.twig` after `AB_<file>_label` if the catalog carries one.
     */
    ...
    /**
     * The other direction: a key a Component names but no catalog defines renders as its own name in the rail, which is how `AB_advanced_action_textsearch_label` looked fine for as long as it did.
     */
