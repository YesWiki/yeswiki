...
/** The link editor is a rail beside the editor, on the same rules as the actions builder: it opens on the link you point at, closes when you leave it, and writes into the document only when asked. */
...
/** The page suggestions are a dropdown, and a dropdown needs a way out. */
...
/** The source editor's own way in, which the wysiwyg one has no equivalent of: there, a link is written by selecting words and pressing a toolbar button, and the selection is captured when the button is pressed -- the rail is not a modal, so nothing stops the caret moving on before the link is filled in. */
...
/** ...and there, the rail still opens on whatever the caret lands in. */
