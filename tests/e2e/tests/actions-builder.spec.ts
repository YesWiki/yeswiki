...
test('the builder is a rail down the right-hand side', async({page}, testInfo) => {
  ...
  expect(panel.x + panel.width, 'it is docked against the right edge').toBeGreaterThan(viewport.width - 2)
  expect(panel.x, 'and it is a rail, not a sheet over everything').toBeGreaterThan(viewport.width / 2)
...
test('closing the rail slides it back off the right edge', async({page}) => {
  await login(page, ADMIN_USERNAME, ADMIN_PASSWORD)
  await openEditor(page)
  const offset = () => page.evaluate(() => new DOMMatrix(
    getComputedStyle(document.getElementById('actions-builder-panel')).transform
  ).m41)
  ...
  await expect(page.locator(PANEL)).toBeHidden()
  await expect.poll(offset, {message: 'and back out again'}).toBeGreaterThan(0)
})
 ...
 */
test('inserting from the rail writes the action and stays on it', async({page}) => {
  await login(page, ADMIN_USERNAME, ADMIN_PASSWORD)
  await openEditor(page)
  await openBuilder(page)
...
test('the rail opens on a palette of every component', async({page}) => {
  ...
  expect(total, 'a wiki ships far more than a handful of components').toBeGreaterThan(20)
  // grouped, and every card names itself
  await expect(page.locator(`${PANEL} .actions-builder-panel__palette-group`).first()).toBeVisible()
  await expect(page.locator(`${COMPONENT} .actions-builder-panel__component-label`).first()).not.toBeEmpty()
  ...
  await expect.poll(async() => page.locator(COMPONENT).count()).toBeLessThan(total)
  await page.locator(`${PANEL} input[type="text"]`).first().fill('no component is called this')
  await expect(page.locator(COMPONENT)).toHaveCount(0)
  await expect(page.locator(`${PANEL} .actions-builder-panel__empty`)).toBeVisible()
})

test('picking from the palette opens its settings, and back returns', async({page}) => {
  await login(page, ADMIN_USERNAME, ADMIN_PASSWORD)
  await openEditor(page)
  await openBuilder(page)

  const firstLabel = await page.locator(`${COMPONENT} .actions-builder-panel__component-label`).first().textContent()
  ...
  await expect(page.locator(`${PANEL} .yw-rail__title`)).toHaveText(firstLabel.trim())
  await expect(page.locator(`${PANEL} select[data-yw-action-select]`)).toHaveCount(0)

  // still choosing what to add, so the palette is behind these settings
  await page.locator(BACK).click()
  await expect(page.locator(PALETTE).first()).toBeVisible()
  await expect(page.locator(`${COMPONENT} .actions-builder-panel__component-label`).first())
 ...
 */
test('putting the cursor in a component opens its settings', async({page}) => {
  await login(page, ADMIN_USERNAME, ADMIN_PASSWORD)
  await openEditor(page)
  ...
  await expect(page.locator(PANEL)).toBeVisible()
  await expect(page.locator(PALETTE), 'it edits that component, it does not offer new ones').toHaveCount(0)
 ...
 */
test('moving the cursor out of a component closes the rail, changing nothing', async({page}) => {
  await login(page, ADMIN_USERNAME, ADMIN_PASSWORD)
  await openEditor(page)
  ...
  await putCursorAt(page, 0, 12)
  await page.locator(`${PANEL} input[value="Hello"]`).fill('Edited but never asked for')
  await putCursorAt(page, 1, 4)

  await expect(page.locator(PANEL)).toBeHidden()
  await expect(page.locator(MARKER), 'and the text is no longer marked').toHaveCount(0)
  expect(await editorContent(page), 'nothing was written on the way out').toBe(text)
})

test('moving from one component to the next swaps the rail onto it', async({page}) => {
  await login(page, ADMIN_USERNAME, ADMIN_PASSWORD)
  await openEditor(page)
  await replaceEditorTextNewContent(page, [
 ...
 */
test('the toolbar button adds a component after the one the cursor is in', async({page}) => {
  await login(page, ADMIN_USERNAME, ADMIN_PASSWORD)
  await openEditor(page)
  ...
  await expect(page.locator(PALETTE).first(), 'the button offers components, it does not edit').toBeVisible()
  await expect(page.locator(MARKER), 'and it is on none of them yet').toHaveCount(0)
  await pickFromPalette(page, 'Bouton')
  await page.locator(SUBMIT).click()

  await expect.poll(async() => (await editorContent(page)).split('\n').length).toBeGreaterThan(1)
  const lines = (await editorContent(page)).split('\n')
  expect(lines[0], 'the component the cursor was in is untouched').toBe(existing)
...
test('updating from a closing tag rewrites the opening one', async({page}) => {
  await login(page, ADMIN_USERNAME, ADMIN_PASSWORD)
  await openEditor(page)
  await replaceEditorTextNewContent(page, [
    '{{section height="111px"}}', 'du texte', '{{end elem="section"}}'
  ].join('\n'))
  ...
  expect(lines[2], 'the closing tag is left alone').toBe('{{end elem="section"}}')
})
