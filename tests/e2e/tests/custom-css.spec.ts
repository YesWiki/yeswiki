 ...
 * every unit test.
 */
test('an admin writes the wiki stylesheet and it reaches an ordinary page', async({page}) => {
  ...
  await page.evaluate(() => {
    window['ace'].edit(document.querySelector('.yw-custom-css__ace')).setValue('')
  })
  await expect(box).toHaveValue('')

  // the rail names the section it belongs to
  await expect(page.locator('.yw-dashboard__sidebar')).toContainText('Apparence')
  await expect(page.locator('.yw-dashboard__link--current')).toContainText('CSS')
  ...
  await expect(page.locator('.yw-custom-css__ace .ace_support.ace_type').first()).toBeVisible()
  ...
  const link = page.locator('link[rel="stylesheet"][href*="custom/styles/custom.css"]')
  ...
  await expect.poll(async() => page.locator('.e2e-probe').evaluate((el) => getComputedStyle(el).color))
    .toBe('rgb(1, 2, 3)')
  ...
  await page.locator('button[type="submit"]').click()
  await page.goto('/?PagePrincipale')
  await expect(page.locator('link[href*="custom/styles/custom.css"]')).toHaveCount(0)
})

test('the stylesheet screen is refused to someone who is not an admin', async({page}) => {
  await page.goto('/?admin/custom-css')
