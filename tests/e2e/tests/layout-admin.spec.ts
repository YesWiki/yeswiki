 ...
 */
test('what an admin types into Layout becomes the navbar of every page', async({ page }) => {
  ...
  await expect(page.locator('.yw-dashboard__link--current')).toContainText('Mise en page')
  ...
  await expect(added.locator('input[name$="[label]"]'))
    .toHaveAttribute('name', `navbar[${before}][label]`)
  ...
  await expect(page.locator('.navbar-brand').first()).toContainText('Le wiki du collectif')
  await expect(page.locator('.yw-topnav')).toContainText('Le forum')
  await expect(page.locator('.yw-topnav a[href="https://forum.example"]')).toBeVisible()
})
...
test('a dropdown is an entry with children under it', async({ page }) => {
  ...
  await expect(page.locator('[data-yw-layout-rows="navbar"] .yw-layout__row--child').first())
    .toBeVisible()
 ...
 */
test('the logo button opens the file rail, and Remove clears it', async({ page }) => {
  await login(page, ADMIN_USERNAME, ADMIN_PASSWORD)
  await page.goto('/?admin/layout')

  // the value lives in a hidden field; there is no text box to type an address into
  await expect(page.locator('#yw-layout-logo')).toHaveAttribute('type', 'hidden')
  await expect(page.locator('input[name="layout_logo"][type="text"]')).toHaveCount(0)
 ...
 */
test('the top bar previews what is being typed, and saves nothing until Save', async({ page }) => {
  await login(page, ADMIN_USERNAME, ADMIN_PASSWORD)
  await page.goto('/?admin/layout')
  ...
  await expect(chrome.locator('.navbar-brand')).toContainText('Titre provisoire', { timeout: 10000 })
  ...
  const applied = await page.evaluate(() =>
    getComputedStyle(document.documentElement).getPropertyValue('--yw-navbar-height').trim())
  expect(applied).toBe('96px')

  // ...and none of it was saved: another page still wears the wiki's real chrome
  await page.goto('/?PagePrincipale')
  await expect(page.locator('#yw-layout-chrome .navbar-brand')).not.toContainText('Titre provisoire')
  const saved = await page.evaluate(() =>
    document.documentElement.style.getPropertyValue('--yw-navbar-height').trim())
  expect(saved, 'the height is the saved one, not the one dragged to').not.toBe('96px')
})

test('the bar height is saved, and beats what a stylesheet says', async({ page }) => {
  await login(page, ADMIN_USERNAME, ADMIN_PASSWORD)
  await page.goto('/?admin/layout')
  ...
  await expect(page.locator('html')).toHaveAttribute('style', /--yw-navbar-height:\s*88px/)
  ...
  const barHeight = await page.evaluate(() =>
    Math.round(document.querySelector('#yw-topnav').getBoundingClientRect().height))
  expect(barHeight, 'the number on the slider is the height of the bar').toBeLessThanOrEqual(90)
  expect(barHeight, 'the number on the slider is the height of the bar').toBeGreaterThanOrEqual(88)
  ...
  const railTop = await page.evaluate(() =>
    Math.round(document.querySelector('.yw-dashboard__sidebar').getBoundingClientRect().top))
 ...
 */
test('leaving with unsaved changes asks, and typing does not', async({ page }) => {
  await login(page, ADMIN_USERNAME, ADMIN_PASSWORD)
  await page.goto('/?admin/layout')
  ...
  await expect(page.locator('#yw-layout-chrome .navbar-brand'))
    .toContainText('Un titre pas encore enregistré', { timeout: 10000 })
  ...
  await page.locator('.yw-dashboard__sidebar a[href*="admin/preset"]').first().click()
  ...
  await expect(page.locator('#yw-layout-title')).toHaveValue('Un titre pas encore enregistré')
})

test('saving clears the guard, so leaving afterwards asks nothing', async({ page }) => {
  await login(page, ADMIN_USERNAME, ADMIN_PASSWORD)
  await page.goto('/?admin/layout')
...
test('the three chrome pages that are still pages are linked, not replaced', async({ page }) => {
  await login(page, ADMIN_USERNAME, ADMIN_PASSWORD)
  await page.goto('/?admin/layout')
