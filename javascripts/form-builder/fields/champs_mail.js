...
export default {
  ...
  attributes: {
    ...
    see_mail_acls: { ...readConf, ...{ label: _t('BAZ_FORM_EDIT_EMAIL_SEE_MAIL_ACLS') } },
  ...
  // disabledAttributes: [],
  editorSetup(api) {
    const arrayEquals = (a, b) => Array.isArray(a) && Array.isArray(b)
      && a.length === b.length && a.every((e) => b.includes(e))
    // read and readWhenForm mirror each other
    api.onChange('read_access', () => {
      if (!arrayEquals(api.getValue('readWhenForm'), api.getValue('read_access'))) {
        api.setValue('readWhenForm', api.getValue('read_access'), { silent: true })
      }
    })
    api.onChange('readWhenForm', () => {
      if (!arrayEquals(api.getValue('read_access'), api.getValue('readWhenForm'))) {
        api.setValue('read_access', api.getValue('readWhenForm'), { silent: true })
      }
    })
    const applyButtonMode = () => {
      if (api.getValue('show_contact_form') === 'form') {
        ...
        if (arrayEquals(api.getValue('read_access'), ['*']) && !write.includes('*')) {
          api.setValue('read_access', ['%'])
        }
