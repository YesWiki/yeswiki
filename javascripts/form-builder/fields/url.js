...
export default {
  ...
  editorSetup(api) {
    ...
    if (parts.length > 1) {
      if (!api.getValue('ratio'))
        api.setValue('ratio', parts[0] || '', { silent: true })
      if (!api.getValue('maxwidth'))
        api.setValue('maxwidth', parts[1] || '', { silent: true })
      if (!api.getValue('maxheight'))
        api.setValue('maxheight', parts[2] || '', { silent: true })
    }
    const packOptions = () => {
    ...
    const applyVisibility = () => {
      ...
      ;['ratio', 'maxwidth', 'maxheight', 'class'].forEach((attr) =>
        isVideo ? api.show(attr) : api.hide(attr),
      )
      packOptions()
    }
    ;['ratio', 'maxwidth', 'maxheight'].forEach((attr) =>
      api.onChange(attr, packOptions),
    )
