...
const geolocationHelper = (function () {
  const eventDispatcher = {
    ...
    cleanEvent(eventName) {
      this.eventsListeners[eventName] = this.eventsListeners[eventName].filter(
        (eventData) => !eventData.once || !eventData.triggered,
      )
    },
    ...
    async runOnceOrWait(eventId, eventPrefix, asynFunc) {
      if (
        !(eventPrefix in this.loadingCache) ||
        !Array.isArray(this.loadingCache[eventPrefix])
      ) {
      ...
      const p = new Promise((resolve, reject) => {
        ...
        if (!this.loadingCache[eventPrefix].includes(eventId)) {
          ...
          asynFunc()
            ...
            .catch((e) => {
              this.dispatchEvent(errorEventName, e)
            })
        }
      })
    ...
    setEventTriggered(eventName) {
      if (typeof eventName == 'string' && eventName in this.eventsListeners) {
        this.eventsListeners[eventName] = this.eventsListeners[eventName].map(
          (evenData) => {
  ...
  const geolocationHelperInternal = {
      ...
      return {
        ...
        countries: [
          ...
          { id: 'AL', name: 'Albania', box: [19.3, 39.62, 21.02, 42.69] },
          {
            ...
            box: [51.58, 22.5, 56.4, 26.06],
          },
          {
            id: 'AR',
            name: 'Argentina',
            box: [-73.42, -55.25, -53.63, -21.83],
          },
          ...
          { id: 'AQ', name: 'Antarctica', box: [-180.0, -90.0, 180.0, -63.27] },
          {
            ...
            box: [68.72, -49.78, 70.56, -48.63],
          },
          {
            id: 'AU',
            name: 'Australia',
            box: [113.34, -43.63, 153.57, -10.67],
          },
          ...
          { id: 'CM', name: 'Cameroon', box: [8.49, 1.73, 16.01, 12.86] },
          {
            id: 'CD',
            name: 'Congo (Kinshasa)',
            box: [12.18, -13.26, 31.17, 5.26],
          },
          {
            id: 'CG',
            name: 'Congo (Brazzaville)',
            box: [11.09, -5.04, 18.45, 3.73],
          },
          ...
          { id: 'GM', name: 'Gambia', box: [-16.84, 13.13, -13.84, 13.88] },
          {
            id: 'GW',
            name: 'Guinea Bissau',
            box: [-16.68, 11.04, -13.7, 12.63],
          },
          {
            id: 'GQ',
            name: 'Equatorial Guinea',
            box: [9.31, 1.01, 11.29, 2.28],
          },
          ...
          { id: 'PH', name: 'Philippines', box: [117.17, 5.58, 126.54, 18.51] },
          {
            ...
            box: [141.0, -10.65, 156.02, -2.5],
          },
          { id: 'PL', name: 'Poland', box: [14.07, 49.03, 24.03, 54.85] },
          {
            id: 'PR',
            name: 'Puerto Rico',
            box: [-67.24, 17.95, -65.59, 18.52],
          },
          {
            id: 'KP',
            name: 'North Korea',
            box: [124.27, 37.67, 130.78, 42.99],
          },
          ...
          { id: 'SN', name: 'Senegal', box: [-17.63, 12.33, -11.47, 16.6] },
          {
            ...
            box: [156.49, -10.83, 162.4, -6.6],
          },
          {
            id: 'SL',
            name: 'Sierra Leone',
            box: [-13.25, 6.79, -10.23, 10.05],
          },
    ...
    methods: {
          ...
          Math.sin(
            Math.sqrt(
              Math.sin(latDiff / 2) * Math.sin(latDiff / 2) +
                Math.cos(radiusLatFrom) *
                  Math.cos(radiusLatTo) *
                  Math.sin(lngDiff / 2) *
                  Math.sin(lngDiff / 2),
            ),
          )
      ...
      async geolocate(address) {
        ...
        if (typeof address === 'object' && address !== null) {
          ...
          Object.keys(associations).forEach((dataKey) => {
            ...
            if (
              dataKey in address &&
              typeof address[dataKey] === 'string' &&
              address[dataKey].length > 0
            ) {
          ...
          if (Object.keys(params).length > 0) {
            if (
              'street' in params &&
              (!('postalcode' in params) || !('city' in params))
            ) {
              ...
              url = `https://nominatim.openstreetmap.org/search?${Object.keys(
                params,
              )
        ...
        return await this.eventDispatcher.runOnceOrWait(
          cacheKey,
          'fromAddress',
          async () =>
            this.getJsonWithTimeLimitThenWait(
              url,
      ...
      getCountry(code) {
        const foundData = this.countries.find(
          (countryData) => countryData.id.toLowerCase() === code.toLowerCase(),
        )
        if (foundData) {
      ...
      getCountryData(country) {
        ...
        const foundData = this.countries.find(
          (countryData) =>
            this.normalizeNFDDiacritic(countryData.name) === normalizedCountry,
        )
        if (foundData) {
          return foundData
        }
            ...
            async () =>
              this.getJsonWithTimeLimitThenWait(
                ...
                .then((data) => {
                  ...
                  this.cache[fromCache][id] = this.convertDataFromGouvApi(
                    data,
                    countryCode,
                    country,
                  )
      ...
      async getGelocationDataFromPostalCode(country, postalCode) {
        ...
        return this.getGelocationDataFromGouv(
          country,
          postalCode,
          'fromPostalCode',
      ...
      async getJsonWithTimeLimitThenWait(
        url,
        id,
        minimumTimeToWait = 100,
        timeLimit = 5000,
      ) {
        if (
          !(id in this.loadingUrl) ||
          typeof this.loadingUrl[id] !== 'object'
        ) {
          this.loadingUrl[id] = {
            running: false,
            next: [],
        ...
        const p = new Promise((resolve) => {
          this.loadingUrl[id].next = [...this.loadingUrl[id].next, resolve]
          if (canStartDirectly) {
            processNext()
          }
        })
        return await p
          .then(() => this.getJsonWithTimeLimit(url, timeLimit))
          .finally(() => {
      ...
      async getJsonWithTimeLimit(url, timeLimit = 5000) {
        ...
        return fetch(url, { signal: abortController.signal })
          .then((response) => {
            if (!response.ok) {
              throw new Error(
                'response not ok when getting data from postal code',
              )
            }
      ...
      toGeolocationData(data) {
        ...
        tab.forEach((key) => {
          exportData[key] =
            key in sanitizedData &&
            ['string', 'number'].includes(typeof sanitizedData[key])
              ? String(sanitizedData[key])
              : ''
        })
        exportData.postalCodes =
          'postalCodes' in sanitizedData &&
          Array.isArray(sanitizedData.postalCodes)
            ? sanitizedData.postalCodes
                .map((e) => (['string', 'number'].includes(typeof e) ? e : ''))
  ...
  return {
    calculateGeodesicDistance({
      ...
      return geolocationHelperInternal.methods.calculateGeodesicDistance({
        point1,
        point2,
      })
    },
    ...
    async getGelocationDataFromPostalCode(country, postalCode) {
      return geolocationHelperInternal.methods.getGelocationDataFromPostalCode(
        country,
        postalCode,
      )
    },
    async getGelocationDataFromTown(country, town) {
      return geolocationHelperInternal.methods.getGelocationDataFromTown(
        country,
        town,
      )
    },
