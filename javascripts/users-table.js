    ...
    if (form.length > 0) {
      ...
      $.ajax({
        ...
        success(data) {
          ...
          $(`#${tableId}`)
            .DataTable()
            .row.add(['', userName, '', userEmail, signuptime, ''])
            .draw()
          if (userLink !== '') {
            $('#users-table-link-change-password').html(
              `<br/><label>${_t('LINK_TO_CHANGE_PASSWORD')}</label><br/><a href='${userLink}' target='_blank'>${userLink}</a>`,
            )
          }
          toastMessage(
  ...
  deleteUser(event) {
    ...
    if (!usersTableService.isRunning) {
      ...
      if (elem) {
        ...
        const csrfToken =
          wiki.antiCsrfToken || 'error wiki has not antiCsrfToken'
        ...
        $.ajax({
          ...
          success() {
            ...
            $(modal)
              ...
              .append(
                $('<div>').text(
                  _t('USERSTABLE_USER_DELETED', { username: name }),
                ),
              )
          },
          complete() {
