document.addEventListener('DOMContentLoaded', () => {
  ...
  if (driverSelect) {
    const updateFieldsVisibility = () => {
      ...
      document.querySelectorAll('.db-server-field, .db-server-info').forEach((element) => {
  ...
  const contentChoices = document.querySelectorAll('input[type="radio"][name="contentSQL"]')
  if (contentChoices.length) {
    ...
    contentChoices.forEach((choice) => {
      choice.addEventListener('change', () => updateAdminForm(choice.value !== 'default'))
    })
    const checked = document.querySelector('input[type="radio"][name="contentSQL"]:checked')
