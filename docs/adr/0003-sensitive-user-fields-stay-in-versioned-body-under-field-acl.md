...
We decided **not** to carve sensitive fields (password hash, tokens) into a separate, non-versioned table. Instead they stay hashed in the same versioned `body` as everything else, and are hidden from display via **Field ACL** — the existing per-field read/write ACL mechanism already implemented for bazar entry fields (`FIELD_READ_ACCESS`/`FIELD_WRITE_ACCESS` in `src/Content/Field/BazarField.php`, enforced by `canRead()`/`canEdit()`). Not `metadata`: ADR-0002's amendment narrowed that to presentation and access, and ADR-0011 spells out why the hash could not live there anyway — Field ACL is a property of a template field, so moving the hash out of the body would remove the very mechanism protecting it. Field ACL enforcement must apply uniformly to historical revisions, not just the latest — otherwise the page-history view becomes a way to read old password hashes despite the current view hiding them.
